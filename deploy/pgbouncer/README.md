...
an upstream connection. This deliberately fails fast during pool saturation or
PostgreSQL outages. The shared `@boardsesh/db` client retries the failures that
prove the statement never reached PostgreSQL (SQLSTATE `08P01` with
`query_wait_timeout`, `client_login_timeout (server down)`,
`no more connections allowed (max_client_conn)` or the `server_login_retry`
fast-fail) within its 10-second budget, which leaves room for one retry after a
5-second wait timeout. A wait timeout on a connection that
was already open surfaces as `CONNECTION_CLOSED` and is not retried. Details:
`docs/db-connectivity.md`.

Production values: `PGBOUNCER_UPSTREAM_HOST=postgis---pg18.railway.internal`,
port `5432`, and `PGBOUNCER_SERVER_TLS_CA` set to the Boardsesh DR Primary CA
certificate (public half of 1Password item `Boardsesh DR primary TLS`). The
primary's leaf lists that host as a SAN, so `verify-full` passes only once the
primary serves the private-CA leaf; until then every server login fails. The
PG18 primary allows 100 connections, and this pool takes at most 45 of them.
...
`MIGRATOR_DATABASE_URL` is the dedicated `boardsesh_migrator` credential and
may reach the same direct endpoint as the still-direct runtime `DATABASE_URL`
before cutover. After cutover the URLs naturally differ because only runtime
traffic uses the pooler. Migrations never fall back to the runtime connection
or substitute the shared `DATABASE_DIRECT_URL` credential.
