...
- Both `mobile-screenshots-ios.yml` and `mobile-screenshots-android.yml` are manual dispatch only for now (the nightly cron was removed); an automatic trigger after each native build is coming in a later PR.
