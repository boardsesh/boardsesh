...
- Captures can run against RECORDED backend responses instead of live PROD, so the shots are deterministic: `vp run mobile:screenshot-backend` (record/replay proxy, fixtures under `packages/mobile/screenshot-fixtures`). Full contract, log grammar and keying rules: `docs/mobile-screenshot-fixtures.md`.
- `--fixtures record|replay` (plus `--fixtures-dir`, `--fresh`, `--no-pseudonymise`) starts that backend for the run and points the JS bundle at it; `--backend local|prod` still only names the UPSTREAM a recording proxies. Any replay miss fails the capture, and `packages/mobile/src/lib/graphql/__tests__/screenshot-fixture-drift.test.ts` keeps the committed set honest on every PR (it also validates mobile's whole operation set against the schema). Recording happens in the two capture workflows (`fixtures = record`), then `vp run mobile:screenshot-fixtures-merge` folds the shard artifacts into the one committed set — runbook in the doc above. **A recording replaces every climber but the recording account with a stable stand-in before writing** (names, handles, avatars, emails); `--no-pseudonymise` opts out and a set recorded with it must never be committed. `vp run mobile:screenshot-fixtures-pseudonymise` applies the same rewrite to a set that already exists.
...
<<<<<<< HEAD
- Both `mobile-screenshots-ios.yml` and `mobile-screenshots-android.yml` are manual dispatch only for now (the nightly cron was removed); an automatic trigger after each native build is coming in a later PR. CI captures replay the pinned fixture snapshot by default (`fixtures = replay`); re-record with `fixtures = record`.
||||||| parent of fdd2760e7 (ci(mobile-screenshots): run captures after each native deploy that shipped a binary)
- Both `mobile-screenshots-ios.yml` and `mobile-screenshots-android.yml` are manual dispatch only for now (the nightly cron was removed); an automatic trigger after each native build is coming in a later PR. CI captures replay the committed fixture set by default (`fixtures = replay`); re-record with `fixtures = record`.
=======
...
- CI captures replay the committed fixture set by default (`fixtures = replay`); re-record with `fixtures = record`.
>>>>>>> fdd2760e7 (ci(mobile-screenshots): run captures after each native deploy that shipped a binary)
