...
- `docs/garmin-sideload-build.md` — the Connect IQ pipeline: the rolling `garmin-latest` tag and its stable sideload URLs, why CI downloads the SDK per run instead of baking an image (the licence forbids redistribution), the `Garmin` environment contract, the throwaway-key-on-PR vs real-key-on-release split, and how to add a watch
