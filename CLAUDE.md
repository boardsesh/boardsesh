...
- `docs/rust-backend-migration.md` — the Rust backend epic (`epic:rust-backend`, issues `RB-NN`): the strangler gateway in front of Node, the wire and Redis contracts the port must keep, the validation oracle (conformance suite, SDL parity, differential replay, keyspace diff, soak), the ralph-loop gate contract, the deliberate divergences and the phase table
