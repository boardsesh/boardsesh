...
- `docs/cnc-packs.md` — CNC build packs: the order state machine (the order row is also the generation queue), Stripe Checkout + the webhook contract (which events, why 400/500/200 mean what they mean, the `cnc_stripe_events` idempotency gate), Stripe Tax, the pull-based worker job API (claim/heartbeat/complete/fail, the two credentials, leases and the reaper, the dictated output key), the download route and its five-minute grants, the regenerate runbook, and the fail-closed env vars
