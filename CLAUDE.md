...
- **Ready to merge signal**: once CI is green, no unresolved review comments remain, and there are no conflicts, remove the draft status from the PR marking it ready for review. **An empty check list is not green** — "zero failures, zero pending" is the same shape as "CI never ran" (#4758). Confirm `ci-status` actually reported on the head SHA before calling a PR ready: `gh pr view <n> --json statusCheckRollup --jq '.statusCheckRollup | length'` returning 0 means nothing is attached. Recovery ladder: `docs/ci-required-checks.md`.
...
- `docs/ci-required-checks.md` — why `ci-status` is the one required context on main, the ruleset payload that makes it required (and why a ruleset rather than classic protection, given the bot's direct pushes), and the detect/recover ladder for a PR whose checks never arrived
