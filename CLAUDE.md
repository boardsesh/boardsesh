...
- The default Apple capture is `--devices common --locales all`: iPhone 16 Pro Max, iPhone 14 Plus, and iPhone 16 Pro for store-ready app locales (`en-US`, `es`, `fr`; German `de` is supported in-app but store screenshots are a follow-up — `--locales de` maps to `de-DE`). The Spanish app locale is written to both App Store Connect folders, `es-ES` and `es-MX`.
