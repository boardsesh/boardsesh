...
- The iOS workflow is **self-gating**. A `gate: probe` dispatch captures ONE shard (en-US x iPhone 16 Pro Max) through the shared `.github/actions/ios-screenshot-shard` composite, compares it against the stored baseline with `vp run screenshot:compare` (`scripts/compare-screenshots.ts`; channel tolerance 8, max differing-pixel ratio 0.001, both overridable via `SCREENSHOT_CHANNEL_TOLERANCE` / `SCREENSHOT_MAX_DIFF_RATIO`), and fans out to the other 11 shards only when it changed — or when `scripts/screenshot-probe-scope.ts` (`vp run screenshot:probe-scope`) forces it anyway because a changed path falls in a locale/iPad scope the single probe shard can't see. `gate: full` (the default) always captures everything, and a narrowed `locales`, the `onboarding` flow or `upload: true` force `full`.
- The baseline is a rolling GitHub prerelease tagged `screenshots-baseline` — one zip per (store locale, device) plus `ios-manifest.json` — managed by `vp run screenshot:baseline` (`pack` / `publish` / `fetch`, `scripts/screenshot-baseline.ts`). Captured PNGs stay gitignored (#2905). `fetch` only reports found when the manifest is present and every shard verifies against it (both directions), so a partial or corrupted baseline falls back to a full capture instead of being trusted. Publishing is manual today: dispatch with `publish_baseline: true` (`gate: full`, or `gate: probe` when no baseline exists yet) — the `workflow_run` trigger (a later PR) will publish automatically after each green full capture. `mobile-store-draft.yml` fetches the whole set and attaches it to the App Store version it just created. Full runbook: `docs/mobile-store-release.md`.
