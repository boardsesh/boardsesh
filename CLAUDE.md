...
- `docs/cnc-packs.md` — CNC build packs: the order state machine (the order row is also the generation queue), Stripe Checkout + the webhook contract (which events, why 400/500/200 mean what they mean, the `cnc_stripe_events` idempotency gate), Stripe Tax, and the fail-closed env vars
