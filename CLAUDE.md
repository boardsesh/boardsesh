...
- **Release notes**: every PR description must include the `## Release Notes` section from the PR template. Write in climber voice — describe what the user gets, not what the code does. Internal-only changes (refactor, CI, deps, tests) tick the template's **"No release note needed"** checkbox — `scripts/check-release-notes.ts` looks for that checkbox or the `skip-changelog` label, and rejects the bare word `none` as an empty section.
...
- `docs/production-deploy.md` — the production deploy concurrency group, how a run parked at the Production environment gate wedges every later push, the watchdog that breaks it, and the "main has not deployed" runbook; also `WEB_DEPLOY_TARGETS` (Railway, or `none` for the web hold), the completed Vercel-to-Railway cut-over, and the web rollback runbook — including why setting the hold BEFORE a dashboard rollback is now mandatory, since the `check-rollback` probe that automated it went away with Vercel; also deployment teardown (`drainingSeconds` on both services, why it must be an unquoted number, why the backend closes its listener before the WebSocket teardown, why the web service needs no custom SIGTERM handler, and why `overlapSeconds` stays unset)
...
- `docs/keychain-namespaces.md` — the two iOS keychain namespaces behind every SecureStore key: why the v2 service exists, the v2-first read order, the freshness stamp that tells a stale v2 copy from a fresh one, the tombstone contract, what an OTA rollback and roll-forward each cost, and where phase 2 (#4128) lands
- `docs/db-migrations.md` — migration numbering, `when`-not-number apply order, the collision/renumber bot, when it hands work back, and the out-of-band `CONCURRENTLY` pattern for indexing a large write-hot table
...
- `docs/board-render-analytics.md` — the board-render event contract (issue #2202): the five surviving events (the paired board-look step events, the settings/preset events, and `Board Render Failed`), the common-props builder, why the `Climb View Opened` / `Board Pinch` / `Climb First Action` trio was retired and what that costs (`Board Render Failed` is now an absolute count — its per-view denominator is gone), and the stratification rule (never pool across `board_name` or `glow_falloff_source`)
...
- `docs/postgres-query-costs.md` — what our Postgres queries cost under the 4 GB memory cap: the ranked list of changes that cut it (with prod, replica and estimated numbers kept apart), the recurring jobs that rewrote unchanged data, which reads could move onto the device, what was measured and rejected, and the open owner questions
...
- `docs/railway.md` — the OTA project's Railway config-as-code (`vp run railway:apply`): what it applies vs. reports, how an xprem upgrade became a one-line PR (image + deploy + probe + automatic rollback) and why the image change needs `--allow-image-change`, the weekly stable/beta bump PRs (`vp run ota:image-bump`), why `railway.toml` cannot reach an image-sourced service, the owned-value vs. secret variable contract, and the xprem ClickHouse retention guard
...
- Captures can run against RECORDED backend responses instead of live PROD, so the shots are deterministic: `vp run mobile:screenshot-backend` (record/replay proxy; ignored recordings under `packages/mobile/screenshot-fixtures`, verified replay cache under `.boardsesh/screenshot-fixtures`). Full contract, log grammar and keying rules: `docs/mobile-screenshot-fixtures.md`.
- `--fixtures record|replay` (plus `--fixtures-dir`, `--fresh`, `--no-pseudonymise`) starts that backend for the run and points the JS bundle at it; `--backend local|prod` still only names the UPSTREAM a recording proxies. Any replay miss fails the capture, and `packages/mobile/src/lib/graphql/__tests__/screenshot-fixture-drift.test.ts` checks the pinned snapshot on every PR (it also validates mobile's whole operation set against the schema). Recording happens in the two capture workflows (`fixtures = record`), then `vp run mobile:screenshot-fixtures-merge` folds the shard artifacts into one sanitized recording; `vp run mobile:screenshot-fixtures-publish` uploads it to the dev S3 bucket. Commit only `app-stores/screenshot-fixtures.json` — runbook in the doc above. **A recording replaces every climber but the recording account with a stable stand-in before writing** (names, handles, avatars, emails); `--no-pseudonymise` opts out and a set recorded with it must never be published. `vp run mobile:screenshot-fixtures-pseudonymise` applies the same rewrite to a set that already exists.
- Both workflows run **automatically after each native deploy on `release/next` that shipped a binary** (the release train is where store candidates are built), and on a manual dispatch from any branch — nothing else, no cron. A green deploy run is not proof: a JS-only push completes it with the build job skipped, so each run first checks that a `fingerprint-<platform>-*` tag points at the triggering commit (`workflow_run.head_sha`) and skips with a step-summary line when none does. Every checkout pins that same commit. A forced rebuild of an already-shipped fingerprint keeps the old tag, so it reads as not-shipped — dispatch by hand for those pixels.
- The iOS workflow is **self-gating**. Automatic runs always probe; a `gate: probe` dispatch does the same. It captures ONE shard (en-US x iPhone 16 Pro Max) through the shared `.github/actions/ios-screenshot-shard` composite, compares it against the stored baseline with `vp run screenshot:compare` (`scripts/compare-screenshots.ts`; channel tolerance 8, max differing-pixel ratio 0.001, both overridable via `SCREENSHOT_CHANNEL_TOLERANCE` / `SCREENSHOT_MAX_DIFF_RATIO`), and fans out to the other 11 shards only when it changed — or when `scripts/screenshot-probe-scope.ts` (`vp run screenshot:probe-scope`) forces it anyway because a changed path falls in a locale/iPad scope the single probe shard can't see. On a dispatch, `gate: full` (the default) always captures everything, and a narrowed `locales`, the `onboarding` flow or `upload: true` force `full`. Only a run that captured the complete set uploads to App Store Connect or refreshes the baseline.
- The baseline is a rolling GitHub prerelease tagged `screenshots-baseline` — one zip per (store locale, device) plus `ios-manifest.json` — managed by `vp run screenshot:baseline` (`pack` / `publish` / `fetch`, `scripts/screenshot-baseline.ts`). Captured PNGs stay gitignored (#2905). `fetch` only reports found when the manifest is present and every shard verifies against it (both directions), so a partial or corrupted baseline falls back to a full capture instead of being trusted. An automatic run publishes it after a green full capture; a dispatch asks with `publish_baseline: true` (`gate: full`, or `gate: probe` when no baseline exists yet). `mobile-store-draft.yml` fetches the whole set and attaches it to the App Store version it just created. Full runbook: `docs/mobile-store-release.md`.
...
- An automatic Android run captures, posts the Discord preview and uploads the artifact; committing the set back to `main` stays opt-in on a dispatch (`commit_to_main`), because the Android capture isn't byte-deterministic yet.
- CI captures replay the pinned S3 fixture snapshot by default (`fixtures = replay`); re-record with `fixtures = record`.
- `EXPO_PUBLIC_SCREENSHOT_NOW` freezes "now" in screenshot mode (`packages/mobile/src/lib/clock.ts`), so relative timestamps ("3h ago", day dividers, the activity heatmap window) render identically across repeated captures of the same seeded backend data. The orchestrator sets it whenever `--fixtures` is on, from the fixture set's `frozenNow` (the manifest's on a replay, minted to the second on a recording); a `--fixtures off` capture still runs on the live clock. The value is mid-day UTC (`…T12:00:00Z`) and the timezone is pinned to UTC on both platforms (`SIMCTL_CHILD_TZ` on iOS, `-timezone UTC` on the emulator), so local-date derivations land on the same calendar day everywhere. A replay capture fails if the app's boot log says `clock: live` or names a different instant.

### Android emulator screenshots (local, dev-client + Metro)

For quick ad-hoc shots of the live app on an Android emulator (the fast KVM path on Linux/Intel), `vp run mobile:android-shots` (`scripts/mobile-android-shots.ts`) boots an x86*64 emulator, installs a cached **dev-client** APK (`com.boardsesh.app.dev`, universal `arm64-v8a`+`x86_64`), starts Metro, and captures with `adb exec-out screencap`. Same `EXPO_PUBLIC_SCREENSHOT*_`env as iOS; the deep-link scheme is`com.boardsesh.app://`for both variants (only the package differs). The APK comes from the latest`rn-android-dev-_`release (or a local Gradle fallback). One-time setup:`vp run mobile:android-doctor`(bootstraps the SDK + JDK 21 under`~/.cache/boardsesh/`). Full guide: `docs/android-emulator-screenshots.md`. CI's Play Store capture (`mobile-screenshots-android.yml`) now runs this same dev-client path (`vp run mobile:screenshots --platform android --dev-client`, latest `rn-android-dev-*` APK + Metro); the standalone-APK path (`vp run mobile:screenshots --platform android` without `--dev-client`) remains available for local checks against a store APK with bundled JS.
