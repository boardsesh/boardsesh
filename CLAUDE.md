...
- `docs/licensing.md` — the two-licence split: Apache-2.0 for the repo, AGPL-3.0-or-later for the Aura renderer (`packages/board-renderer`, the Expo `board-renderer` module, `@boardsesh/board-render`, `board-look`, `board-art-geometry`), which generated artifacts are covered, why `pipeline.ts`/`background.ts` carry no SPDX header, and the open legal questions; `REUSE.toml` is the machine-readable map and `scripts/__tests__/licence-boundary.test.ts` guards it
- `docs/renderer-contributor-policy-proposal.md` — proposal (not in force) for DCO / contributor licence grant on the AGPL renderer packages, and why outside renderer PRs are held until it is decided
