...
- `docs/ci-runners.md` — CI capacity: why the org is on the GitHub Team plan and not on a cloud runner pool (the measured queue numbers, the cost table for RunsOn/Blacksmith/Depot/Ubicloud/Hetzner), the single `guards` job that replaced eleven one-minute ci.yml jobs, the queued-age `ci-lane-watchdog`, and `scripts/ci/queue-report.ts` for re-measuring; batch data jobs leave Actions via #5800
