...
- `EXPO_PUBLIC_SCREENSHOT_NOW` freezes "now" in screenshot mode (`packages/mobile/src/lib/clock.ts`), so relative timestamps ("3h ago", day dividers, the activity heatmap window) render identically across repeated captures of the same seeded backend data. Not wired up yet — it will be set by the orchestrator once the replay backend lands, from the recorded fixture set's `frozenNow`; unset today, so captures still run on the live clock. When set, the value must be mid-day UTC (e.g. `…T12:00:00Z`) so local-date derivations (day dividers, the heatmap's today cell) land on the same calendar day in every simulator timezone.
