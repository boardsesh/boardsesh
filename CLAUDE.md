...
- Both workflows run **automatically after each native deploy on `main` that shipped a binary**, and on manual dispatch — nothing else, no cron. A green deploy run is not proof: a JS-only push completes it with the build job skipped, so each run first checks that a `fingerprint-<platform>-*` tag points at the triggering commit (`workflow_run.head_sha`) and skips with a step-summary line when none does. Every checkout pins that same commit. A forced rebuild of an already-shipped fingerprint keeps the old tag, so it reads as not-shipped — dispatch by hand for those pixels.
