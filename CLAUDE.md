...
- `EXPO_PUBLIC_SCREENSHOT_NOW` freezes "now" in screenshot mode (`packages/mobile/src/lib/clock.ts`), so relative timestamps ("3h ago", day dividers, the activity heatmap window) render identically across repeated captures of the same seeded backend data.
