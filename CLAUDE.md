...
- `LICENSING.md` (repo root) — the mixed licence model: AGPL-3.0-or-later for the product core, Apache-2.0 for the interoperability tier (schema, API clients, board catalogue/config, ble-protocol, crypto, moonboard-ocr, the sync packages' `src/api/`, firmware); the Aura renderer scope; the transition point (last Apache-only commit and release); contributions received under Apache-2.0 that stay Apache inside the AGPL work; why `pipeline.ts`/`background.ts` carry no SPDX header; the open legal questions. `REUSE.toml` is the machine-readable map, `LICENSES/` holds both texts, and `scripts/__tests__/licence-boundary.test.ts` guards all of it (including that Apache packages never depend on AGPL ones)
- `docs/contributor-policy-proposal.md` — proposal (not in force) for DCO / contributor licence grant on the AGPL core, the app-store additional-permission direction, and why outside core PRs are held until it is decided
