...
Tests live in `packages/mobile/src/lib/__tests__/`: `secure-store-namespace.test.ts` (phase-1 behaviour + sign-out), `secure-store-rollforward.test.ts` (#5345 roll-forward, the resurrection guard, delete read-back), `secure-store-stamp.test.ts` (the pure rule), `keychain-namespace-migration.test.ts` (the pass), `secure-store-non-ios.test.ts` (Android/web stay single-namespace), `keychain-namespace-migration.web.test.ts` (why `auth-store.web.ts`'s deferred-reconcile retry is allowed to be a no-op — nothing on web records or reports a deferral), `secure-store-writers.test.ts` (every writer routes through the helpers).
