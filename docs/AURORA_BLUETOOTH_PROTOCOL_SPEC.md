...
- **Write type depends on the controller-box generation** — it is a property of the box's advertised characteristic, not a protocol invariant. Two generations exist in the field:
  - The common box (field telemetry `bleCharProperties=12`, API level 3) advertises **both** `.write` (bit 8) and `.writeWithoutResponse` (bit 4); Boardsesh drives it with **Write Without Response** (the fast path).
  ...
  - The official Kilter app sidesteps the problem by driving **every** box with write-with-response, **unconditionally** — confirmed from the arm64 method bodies (`writeData` calls `flutter_blue_plus`'s `write()` with no `withoutResponse` argument; the compiled snapshot's object pool contains only the `withResponse` enum instance, the `withoutResponse` variant being tree-shaken out). It does **not** read the characteristic to decide, and it never negotiates MTU (fixed 20-byte chunks, 100 ms apart). Boardsesh keeps without-response as the fast default and is more surgical about switching: Aurora writes start on without-response, then switch this connection to write-with-response after a without-response write demonstrably stalls on a `.write`-only characteristic and a with-response retry drains. That behavior gate avoids treating a stale CoreBluetooth property read as the live decision.
  - Boardsesh **also** starts on write-with-response from the first connect when the advertised name is a **bare Aurora name with no `#serial@apiLevel` suffix** (`isKilterBuiltBox`) — the signature of the write-only Kilter-built box — so it lights on the first attempt instead of eating a stall. This is a **name** signal, not the property bit, so a healthy serial'd box is never forced onto with-response (the #3228 failure mode). For a bare-name Kilter box Boardsesh also mirrors the app's chunk pacing — 100 ms between with-response chunks on top of the ack; an Aurora box on the with-response fallback stays ack-only (no fixed delay).
