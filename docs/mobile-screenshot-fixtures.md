...
A replay miss answers **HTTP 200 with a GraphQL `errors` array**, never a 5xx.
The app flips its connectivity store into "backend unreachable" on a 5xx or an
`INTERNAL_SERVER_ERROR` extension (`packages/mobile/src/lib/graphql/client.ts`),
which would put an offline banner over the screen being captured and hide the
very miss the log is reporting.
...
packages/mobile/screenshot-fixtures/
  manifest.json
  graphql/<OperationName>/<variablesHash12>.json
  static/<key16>.<ext>
```
...
then query — so re-recording an unchanged capture produces no diff.
...
the run at exit. Replay hands out `screenshot-replay.<hash>` /
`screenshot-replay-refresh` instead, so a fixture set is safe to commit.
...
sends. Both land in follow-up PRs.
