...
Idle warming uses the reference workload in a separate process. The measured process stays on Home apart from the matching background transitions. Both checkpoint requests and sample completions are matched against the reference schedule; a difference exceeding two seconds is rejected. The selected manifest and surface must match the reference.
