...
The plan for replacing `packages/backend` (Node: GraphQL Yoga + graphql-ws + ioredis + drizzle) with a Rust service, one operation at a time, without changing anything a client can observe. Tracked by the `epic:rust-backend` GitHub epic; child issues are `RB-NN`. This page is the architecture and the parity contract those issues point at. It is written for the agents and reviewers working the loop, so it states rules, not intentions.

## Why

Railway bills by memory. On the September 7 baseline (`railway-cost-reduction.md`) the backend's five replicas averaged 2.84 GB RSS together, about 570 MB each, with 30-day spikes near 12 GB (`logging.md`, "Backend memory samples"). Replicas were cut to three the same day; each is capped at 6 vCPU and 8 GB. The backend line was $15.38 of $89.69 incurred that cycle. PostGIS was $57.48 and stays the largest line whatever happens here.
...
| Measure | Node today | Target |
| --- | ---: | ---: |
| RSS per replica at production connection count | ~570 MB average | ≤ 128 MiB |
| RSS per idle authenticated socket | measured in RB-06 | ≤ 20 KiB |
| Replicas | 3 (was 5) | 2 |
| Container image | 2.6 GB dependency layer, `tsx` at runtime | ~30 MB static binary |

Expected saving once realtime is native: roughly $12–14 a month at current prices. That is most of the backend line and about 15% of the bill. The other gains are a backend whose behaviour is pinned by a conformance suite, a Redis keyspace diff and a soak harness, none of which exist today.
...
| Area | Size | Notes |
| --- | ---: | --- |
| GraphQL resolvers | 46.4k lines | 162 Query, 144 Mutation, 9 Subscription root fields. SDL about 9.7k lines and 453 types: 6 unions, no interfaces, one custom scalar `JSON`, about 40 enums with mixed-case values |
...
| Tests | 361 files, 122k lines | About 14 suites drive a real server over HTTP or WebSocket; the rest call resolvers with mocks |
...
4. **GraphQL over WebSocket.** Rust owns the socket, the handshake, auth and the per-connection context. For each `subscribe` message: a `native` root field runs against the local schema; a stateless unported operation (one that needs only `userId` and `clientIp`, such as `saveClimb`) uses `relay`, which is an HTTP POST to Node's `/graphql` carrying the connection's bearer token and forwarded IP headers, with `{data, errors}` mapped to `next` + `complete`; a stateful unported operation (a subscription, or a session-scoped mutation) uses `relay-ws`, a lazily opened upstream graphql-ws connection to Node that reuses the client's `connectionParams` and forwards `next`, `error` and `complete` under the client's operation id, mapping upstream close codes. `relay-ws` exists only until RB-20 and is deleted in RB-22.
5. **Route table.** Compiled defaults per phase, overridable at run time through the Redis hash `rb:routes` (root field → mode), so a bad native operation flips back to `relay` in seconds without a deploy. Introspection relays to Node until the schema is complete.
6. **Rollback ladder.** First, flip `rb:routes` to all-`relay`: Rust becomes a transparent proxy within seconds. Second, redeploy the previous Rust image. Last, move the custom domain back to the Node service; that is a Railway domain re-verification (CNAME plus `_railway-verify` TXT) and takes tens of minutes, and Cloudflare's `ws` record is `proxied-only` with its target managed outside config-as-code. The runbook lives in `production-deploy.md` once RB-05 lands.

## Validation

1. **Conformance suite.** The transport-level vitest suites (`integration`, `queue-client-session`, `session-roster-seed`, `session-subscription-auth`, `redis-pubsub`, `og-climb`, `cors`, `auth-transport`, `session-state`, `board-render`, `websocket-anon-connection-cap`, `climb-stats-subscriptions`, `graphql-resolvers` and the rest that call `startServer()`) get a `BACKEND_UNDER_TEST_URL` switch. Unset, they start Node in-process as today; set, they hit the URL. Their hard-coded ports (8082, 8084) become `TEST_PORT ?? 0`. The same suites run against Node (must stay green) and against Rust (the gate): `vp run test:backend:conformance [--suite <name>]`.
...
4. **Redis cross-implementation tests.** For every `eval` site and every channel: Node writes and Rust reads, and the reverse, against the real Redis from `docker-compose.test.yml`. A scripted session scenario runs on both; the keyspace (`boardsesh:*`, `session:*`, `board:*`, `ratelimit:*`) is dumped and diffed, JSON-valued hash fields structurally with `null` equal to absent except for declared tri-state fields, scalar fields byte for byte, ids and timestamps normalised. Node-side readers (`liveSessions`, `nearbySessions`, `sessionLiveness`) and Node-side writers that keep publishing into realtime channels during phases 1 and 2 (`updateSession` → `SessionNameChanged`, the notification worker, comments, gym notifications, the climb-stats publisher, the REST widget mirror through the shared CAS) each get a case.
...
6. **Load and soak.** `bs-parity soak`: N connections, subscriptions, a mutation rate, protocol pings and reconnect churn. Run against Node first (RB-06) to record the per-connection RSS baseline, then against Rust. The targets in the table at the top are asserted against the `backend.memory` line.
...
- **Two lanes.** `rb:gate` (iteration lane, under 6 minutes warm): preflight (infrastructure up, ports free, database reachable), `cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test -p <crates the task names>`, `bs-parity sdl`, only the conformance suites and replay set the task names, `cargo deny check licenses bans sources`. `rb:gate:full` (nightly and PR CI): full `cargo test --workspace`, full conformance, full cross-implementation matrix, `cargo deny check advisories`, soak. Each ends with exactly one line: `GATE: pass`, `GATE: fail <step>` or `GATE: infra <reason>`.
...
- **Spin guards.** The runner retries `GATE: infra` without invoking the agent. After two consecutive `GATE: fail` on one task it marks the task `blocked` in `PROGRESS.md` and moves on. A path-scope check fails the gate if an iteration touched `packages/backend/src/__tests__/**`, `parity/**/allowlist*`, `scripts/rb-gate*` or another task's checkbox, unless the task's Scope names that path. That stops an agent from "fixing" the oracle.
...
- **CI.** A `backend-rust` job modelled on `renderer-rust` but reusing `test-backend`'s pinned PostGIS and Redis service block, gated on `packages/backend-rs/**`, wired into `ci-status`, with the commit-lint job gated on the same filter.
...
Issue numbers are filled in as the child issues are created.

| Phase | Issues | What lands |
| --- | --- | --- |
| 0 Foundations | RB-01 workspace + CI, RB-02 Node-side prep, RB-03 config + observability + lifecycle, RB-04 pure relay gateway, RB-05 image + deploy topology, RB-06 soak tool + Node baseline | A Rust service that proxies everything, deployed beside Node, with the oracle wired up |
| 1 Realtime | RB-10 Redis + Lua, RB-11 pub/sub + replay + Streams, RB-12 auth, RB-13 graphql-transport-ws server, RB-14 GraphQL core, RB-15 room manager + distributed state, RB-16 sessions, RB-17 queue, RB-18 board presence, RB-19 controller, RB-20 simple subscriptions, RB-21 APNs, RB-22 cutover | Every socket terminates in Rust; replicas 3 → 2; the memory saving |
| 2 GraphQL HTTP domains | RB-30 users + favorites + sync, RB-31 playlists, RB-32 ticks, RB-33 climbs, RB-34 beta videos, RB-35 boards + gyms, RB-36 feeds + follows + comments, RB-37 notifications on Streams, RB-38 proposals + roles + cron, RB-39 spray walls, RB-40 qa + feedback + integrations, RB-41 the Next.js `/api/v1` routes | Each domain relay → shadow → native |
| 3 REST | RB-50 native auth, RB-51 uploads + static, RB-52 OG + board render on board-renderer-core, RB-53 Aurora and Kilter credentials + imports, RB-54 the remaining handlers | No sharp, no WASM |
| 4 Decommission | RB-60 sweeps + email + pg-boss duties, RB-61 remove the relay and Node | `packages/backend` deleted |
