...
4. Application traffic (the Railway web service and the backend) connects directly today. The application's `postgres-js` clients are configured with `prepare: false` (see `packages/db/src/client/postgres.ts`) — this keeps it ready for a transaction-pooling proxy. Without it you get intermittent `prepared statement "X" already exists` errors when PgBouncer reuses backend connections across transactions. The flag is a safe no-op against direct PostgreSQL.
   - **Parked (#4842):** Railway exposes no managed pooled URL. We built a PgBouncer 1.25.2 image (#4849) but parked it on 2026-09-26, never deployed: one Railway web container and ~50 of 97 connections do not need it. Why, and when to revisit: [db-connectivity.md](./db-connectivity.md#pgbouncer-parked).
5. Lock down access:
