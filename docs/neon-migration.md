...
4. For application traffic (the Railway web service and the backend), use a **pooled** URL backed by PgBouncer in transaction-pooling mode. The application's `postgres-js` clients are configured with `prepare: false` (see `packages/db/src/client/postgres.ts`) — this is required for transaction-pool mode. Without it you get intermittent `prepared statement "X" already exists` errors when PgBouncer reuses backend connections across transactions. The flag is a safe no-op against direct PostgreSQL.
   - **Resolved (#4842):** Railway exposes no managed pooled URL, and we do not use the `bitnami/pgbouncer` template. We build our own PgBouncer 1.25.2 image from `deploy/pgbouncer/` (transaction pooling, 45 server connections, verify-full TLS to the PG18 primary). Topology, what stays direct, and the connection budget: [db-connectivity.md](./db-connectivity.md#pgbouncer-in-front-of-production-4842).
5. Lock down access:
