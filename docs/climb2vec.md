...
| **3a** | **Similarity data layer** (paused)              | `board_climb_similar` + numpy top-K (`similarity_export.py`) + `load-similarity.ts`. No resolver reads the table yet, so the weekly workflow stopped rebuilding it (763 MB, zero index scans). Run both scripts by hand when the `similarClimbs` blend (Jaccard fallback) lands. |
