...
| Live stat write-through (`writeClimbStatsEvent`)          | its own native connection (`withExclusiveTransactionAsync`) | one statement per stream event, `BEGIN IMMEDIATE` with a 250ms timeout; a lost lock drops the event and the next pull heals the row            |
