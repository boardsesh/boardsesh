...
**Local-first browse (live).** Climb **search + count + detail** are **local-first**: whenever the active board's exact scope is downloaded and the filters are on-device-expressible, they read local `board_climbs`/`board_climb_stats` (`search-climbs-local.ts` / `get-climb-local.ts`, mirroring the server's LEFT-JOIN standard search) **even while online** — a local query is far faster than a network round-trip. Freshness comes from the background sync (foreground + reconnect), which invalidates `['searchClimbs']`/`['climb']` after each pull so the next local read reflects new data; a downloaded board reads local regardless of connectivity, so connectivity isn't part of the query key. The **network** is used only when there's no usable local data: the board isn't downloaded, or the filter needs a table we don't sync. **Limitations:** filters needing un-synced tables — hold-state (STARTING/HAND/FOOT/FINISH), zone, tall/wide, beta-video — and the drafts path always go to the network (online) or are unavailable (offline); name search is ASCII-case-insensitive only; climb-detail satellites (comments, beta links, similar climbs, stats history) are network-only and absent offline. Trade-off: on a downloaded board, online reads reflect the last sync rather than the live server (acceptable for a climb catalog; the sync keeps it current). **Stats are the exception since #5227:** the layout-wide `climbStatsUpdated` stream writes each recomputed `board_climb_stats` row straight into SQLite and refreshes the affected local-first queries, so the community grade (`display_difficulty`), send count and quality change within a few seconds without a pull-to-refresh. The Boardsesh grade lives in `board_climb_grades` and is not on this stream — it still arrives on the next pull. The default sort is `ascents`, so the default list is stats-dependent and does re-read on a qualifying flush, bounded by the 2 s trailing / 6 s ceiling coalescer. The pull skips server rows younger than the 30 s stability window, so the stream is the prompt local writer for a fresh recompute — including for the tick the device itself just logged. Redis PUBLISH is fail-open, so the periodic reconciliation read writes through the same path and repairs a missed publish. A query the stream must never touch is one served over the network anyway: a scope that isn't downloaded, or a filter SQLite can't express (zone, hold-state, beta-video, drafts) — refreshing those would mean multi-page network refetches every few seconds. See `docs/websocket-implementation.md` for the write's gates and the invalidation rules.
...
| Live stat write-through (`writeClimbStatsEvents`)         | its own native connection (`withExclusiveTransactionAsync`) | one transaction per drain pass (equal-revision republishes settle on an autocommit pre-read and never open it), `BEGIN IMMEDIATE` with a 250ms timeout; a lost lock puts the events back and stops the drain |
