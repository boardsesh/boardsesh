...
Both manual entry points (the user drawer and the More tab) go through `useConfirmSignOut` (`src/hooks/use-confirm-sign-out.ts`), which always confirms and composes an honest message: the downloaded-boards sentence only when `syncEnabledBoards` is non-empty, plus the unsynced-writes count when the queue isn't. It keys off the enabled list rather than the `offline-board-downloads` flag for the same reason the pending-count check is ungated — a flag-rolled-back user can still be holding downloads and still loses them. The confirm lives in the hook, not in `signOut`, so the forced paths and account deletion (already behind its typed-`DELETE` gate) can't inherit it.
