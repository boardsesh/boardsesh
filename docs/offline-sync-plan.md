...
### Feature-flag gate (`offline-board-downloads`)

The offline engine's _new_ work is behind the `offline-board-downloads` PostHog flag, evaluated per user. **The flag is a kill switch, not a rollout gate: it is ON by default and only an explicit `false` turns the engine off** (`isOfflineDownloadsEnabled` returns `featureFlag !== false`). An unresolved flag — PostHog's `/flags` response never landed — reads as ON, because the users it never reaches are the ones who opened the app with no signal, which is this feature's audience. Before issue #4312 an unresolved flag read as OFF, so the offline engine switched itself off for exactly the people it exists for: over 14 days, 1,815 people never once resolved the flag.

Four consequences of that, all deliberate:

- **The kill switch only reaches users who can reach PostHog.** A permanently-offline device keeps the engine no matter what the flag says. That is the intended trade — the alternative is the bug above.
- **It also lands a beat late for a device that _is_ reachable.** `FeatureFlagsProvider` starts every launch with an empty bag, so `useOfflineDownloadsEnabled()` returns `true` on the first render for everyone, including a killed device; the flip to `false` arrives when PostHog answers. If `OfflineSyncBridge`'s scheduler effect wins that race, `startSyncScheduler` has already fired its immediate cycle — and the `stop()` handed back to the effect cleanup unsubscribes triggers, it does not abort a cycle in flight. So "flag off" below describes the steady state, not the first second of a launch: a killed device can still run one drain+pull, with `saveTickOffline` and the favourite dual-write live in that window. Throwing the switch at a crash that happens during engine start-up does not stop it on the launch you throw it.
- **Do not DELETE the flag in PostHog.** A missing key with an empty payload resolves to `undefined`, which now means ON. Disabling the flag or setting it to 0% both yield an explicit `false` and kill the feature correctly.
- **Re-enabling after a long flag-off period can wipe local user data.** `enforceDeletionsCoverage` → `resetUserDataForLostCoverage` `DELETE`s all seven `USER_DATA_TABLES` when the deletions-coverage marker is older than `SYNC_DELETIONS_RETENTION_DAYS - 10`, then rebuilds from the server. Fresh installs are structurally safe (`coverageAt === null` returns early), and nothing can trip it yet given offline shipped 2026-07-17 — but baking the flag on turns "the flag flapped off for a long time" into a local wipe-and-rebuild, and the coverage-reset event does not fire today (#4315), so it would be invisible.

The nested `offline-snapshot-bootstrap-v2` flag keeps its `=== true` default-off, deliberately: starting a download needs connectivity, and connectivity resolves flags within seconds, so the never-resolved cohort barely overlaps with the fresh-download cohort — while default-on would make the snapshot path unkillable for offline-first users while #4313 is open.

Flag **off** keeps pre-offline behavior for everything that runs _online_: the sync scheduler never starts, tick saves/favorite toggles stay network-only, the online local-first read optimization is off (online reads go straight to the network), and the pending-sync badges and Sync-issues section never appear. The **sign-out confirm is not gated** — it asks on every manual sign-out, and simply drops its downloaded-boards sentence when the user has none (which is every flag-off user), so a flag-off dialog says only that they'll need to sign in again. The flag decision is made once in React (`useOfflineDownloadsEnabled`, which includes tester overrides) and mirrored to a module-level store (`src/lib/offline-engine.ts`) for the one non-React consumer, the GraphQL read interceptor. Three things deliberately stay outside the flag: the local SQLite database is still created (empty and idle — gating its creation on an async-loading flag would race); **draining is never gated** — writes queued while the flag was on still flush after a rollback (one-shot drain in `OfflineSyncBridge`, the pending-count-gated sign-out drain, and the Retry button for dead letters); and **reading data already on disk is never gated** (issue #3888) — whenever the network is unavailable, or a request that reached the wire fails (the cold-start race where `onlineManager` hasn't resolved yet, plus captive-portal / dead-upstream), a downloaded board is served from local SQLite and a non-downloaded board gets the per-op empty fallback, regardless of the flag. Same principle as draining: the flag gates _new_ offline work, never reading or draining what's already local.
