...
The entire offline engine is behind the `offline-board-downloads` PostHog flag, evaluated per user. Flag **off** (or not yet loaded) is byte-for-byte pre-offline behavior: the sync scheduler never starts, climb reads/tick saves/favorite toggles stay network-only, the offline read fallbacks are disabled (an offline request errors like it used to), and the pending-sync badges and Sync-issues section never appear. The **sign-out confirm is not gated** — it asks on every manual sign-out, and simply drops its downloaded-boards sentence when the user has none (which is every flag-off user), so a flag-off dialog says only that they'll need to sign in again. The flag decision is made once in React (`useOfflineDownloadsEnabled`, which includes tester overrides) and mirrored to a module-level store (`src/lib/offline-engine.ts`) for the one non-React consumer, the GraphQL read interceptor. Two things deliberately stay outside the flag: the local SQLite database is still created (empty and idle — gating its creation on an async-loading flag would race), and **draining is never gated** — writes queued while the flag was on still flush after a rollback (one-shot drain in `OfflineSyncBridge`, the pending-count-gated sign-out drain, and the Retry button for dead letters).

**Sign-out and queued writes.** A manual sign-out best-effort drains the queue first (bounded at 3s so sign-out never hangs on a bad connection) after the confirm dialog. A **forced** sign-out — the auth interceptor's failed-refresh 401 or checkAuth's proactive-expiry path — deliberately skips both: the token is already dead, so the drain's requests could only fail, and there is no meaningful moment to show a dialog. Queued writes not yet flushed at that point are wiped with the rest of the local data. This is the accepted trade-off for keeping the wipe (a cross-account safety boundary on shared devices) unconditional.

**What sign-out deletes, and the dialog that warns about it.** `purgeLocalDataForSignOut` (`packages/mobile/src/db/connection.ts`) bumps the wipe epoch, clears **every** table in one exclusive transaction — the user's own rows, the mutation queue, **and the downloaded board catalogs** — wipes `sync_meta` whole, then `VACUUM`s so the freed pages actually leave the file. Rows and the `sync_meta` markers describing them must die together: a surviving `scope-complete:` marker would make `isBoardDownloadedLocally` serve an empty catalog to local-first search as a whole board, and a surviving checkpoint would make the strict `>` delta pull resume past rows that are gone. That's why it's a whole-table `DELETE FROM sync_meta` (`deleteAllSyncMeta`) rather than a prefix sweep — the marker families deliberately sit across two prefix conventions. Removing **one** scope while others stay is the opposite problem, and belongs to `scope-teardown.ts`'s exact-key deletes (issue #3617).

Both manual entry points (the user drawer and the More tab) go through `useConfirmSignOut` (`src/hooks/use-confirm-sign-out.ts`), which always confirms and composes an honest message: the downloaded-boards sentence only when `board_climbs` actually holds rows (`hasDownloadedBoardData`, an O(1) `EXISTS`), plus the unsynced-writes count when the queue isn't empty. It probes the rows rather than the `syncEnabledBoards` toggle list or the `offline-board-downloads` flag because the wipe is ungated and the rows are what it deletes — a flag-rolled-back user has an empty toggle list but still-present rows, and they lose them just the same. The confirm lives in the hook, not in `signOut`, so the forced paths and account deletion (already behind its typed-`DELETE` gate) can't inherit it.

Board reference rows used to survive sign-out as a shared cache. That traded a signed-out user's ~200k rows per board of disk for a faster re-enable, and stopped paying once snapshot bootstrap turned a re-download into one CDN GET per board (issue #3621).

### Per-board selective sync

User data (ticks, playlists, favorites, follows) syncs whenever the offline flag is on (no per-board opt-in). Board reference data syncs per-**scope**, where a scope is one `(boardType, layoutId, sizeId)` a user made available offline in **My Boards** (the offline toggle writes an encoded `"boardType:layoutId:sizeId"` key into `syncEnabledBoards`). Downloading always pulls **all sets** for that layout/size — a fixed superset that stays cacheable across users. `syncClimbs`/`syncClimbStats` take optional `layoutId`/`sizeId` args to scope the pull server-side (see the manifest); `sizeId` is ignored for moonboard. Checkpoints are keyed by `(tableName, scopeKey)` so each scope resumes from its own cursor. Sign-out deletes them along with the rows they describe (see **Sign-out and queued writes** above), so the next sign-in re-downloads any board it re-enables.

**Turning the toggle off does not delete anything** — the rows and checkpoints are the expensive shared cache, so re-enabling resumes from the checkpoint instead of re-crawling. Reclaiming that disk space is a separate, explicit action: **More → Storage** (`StorageSettingsScreen`), which lists every scope that has rows (not just the enabled ones — a kill-switch rollback can leave rows behind with the toggle unavailable, so "has rows, not enabled" is a real state, never an orphan to auto-reap). Removal goes through `removeOfflineBoard` (`packages/mobile/src/offline/remove-offline-board.ts`) → `removeBoardScopeData` (`@boardsesh/offline-sync`'s `sync/scope-teardown.ts`), which drops the scope's rows **and every `sync_meta` marker describing them in one exclusive transaction**: a surviving checkpoint would make the strict-`>` delta pull resume past the deleted rows and never revisit them, permanently gutting the catalog while `scope-complete:` still advertised it as whole. See that module's header for the full hazard list. A full `VACUUM` afterwards is what actually returns the pages to the filesystem (`db/vacuum.ts`).
