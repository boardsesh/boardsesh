...
`board_climbs` and `board_climb_hold_sets` both invalidate `['similarClimbs']` and `['holdHeatmap']`. Stats pulls (`board_climb_stats`) deliberately do not invalidate the heatmap: its ascent and grade colours can trail a stats pull by its 5-minute `staleTime`, which is cheaper than re-running a whole-board aggregate on every sync cycle while the overlay is up.
