...
A DB-level `CHECK` constraint (`board_sessions_explicit_board_path_check`) backs up the
application-level guard: `origin <> 'explicit' OR board_path IS NOT NULL`. It doesn't stop an inferred row leaking into a live-session path — that's still
