...
1. **A gym's own listing that carries the serial outranks a personal selection**, and it is checked first. That listing _is_ the wall; a selection is only a hint about it, and most personal boards are the auto-named onboarding template (`Kilter Original 12×12 with kickboard`) that travels with a climber between gyms. The listing has to describe the connected wall to outrank anything — same board type / layout / size / normalized set ids — because the supplier reuses serials across unrelated gyms and a differently-configured wall 7,000 km away says nothing about which board this climber is on. The one selection it does not outrank is **itself**: a climber who picked that gym's listing on the map and plugged into a controller carrying its serial goes straight there, no prompt.
...
3. Otherwise a **previously remembered choice** (`user_board_serials.board_uuid`) wins, so the disambiguation prompt appears once per wall rather than every connect. Except when rule 1's gym listing exists and the remembered board doesn't carry the serial: those links are the wreckage of the old connect path, which wrote whatever board the route was showing, and trusting one would pin a climber to a template forever. Then the remembered link is skipped and the serial's own matches decide.
...
Rule 4's prompt is load-bearing. Serials are not unique — the supplier reuses them across unrelated gyms (production has 15 shared by two gym listings, up to 7,400 km apart), so guessing silently is exactly what made a Bluetooth connect land climbers on the wrong board. `recordBoardSerial` writes the same `board_uuid` column on the same connect, so it defers to the resolver twice over: it omits the column from its conflict `SET` when the connect resolved no board, and fills it only when the stored value is null (`coalesce`) otherwise. The resolver, which is the writer that actually knows which wall the climber is on, always wins.
