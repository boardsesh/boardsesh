...
The per-participant reconnect timer uses the same `SESSION_GRACE_PERIOD_MS` (60 seconds) as the session-memory grace. When a participant's last connection drops, they are marked `RECONNECTING` and a 60-second timer is armed; if no connection re-attaches before it fires, `removeParticipant` is called against distributed state, `UserLeft` is emitted to the remaining members, and the local `sessionParticipants` entry is dropped. Explicit `endSession` cancels these per-participant timers up front so they don't fire against an already-torn-down session.
