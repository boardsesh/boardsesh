...
> **Two supervisors, two jobs — don't confuse them.** `WebSocketConnectionManager` supervises a *live but stale* socket: it watches health events and calls `client.terminate()`, after which graphql-ws's own retry loop reconnects the same client. The kiosk hub supervises the case that outlives that loop — a client whose **retry budget is spent**. graphql-ws is configured with `retryAttempts: 10` and resets its `retries` counter only on a successful `connected`, so once the budget is gone the client is permanently dead and `terminate()` cannot revive it. `KioskPresenceHubInner` therefore starts a `PRESENCE_REBUILD_AFTER_MS` (5 min, one notch above the full retry budget) countdown on `closed`, cancels it on `connected`, and on expiry bumps a `clientGeneration` effect dep so the client is **disposed and replaced** — a fresh client with `retries` back at zero, and every `BoardPresenceProvider` re-subscribing and re-backfilling. Dispose runs before create (React effect cleanup ordering), so a kiosk still holds exactly one socket. This matters only on unattended screens: on a desk the equivalent fix is a page refresh.
