...
The web gym kiosk (`/kiosk/{gym-slug}`, `packages/web/app/components/kiosk/presence/kiosk-presence-hub.tsx`) is a second consumer of the same feed: one graphql-ws client per TV (`connectionName: 'kiosk'`) multiplexes one `boardNowPlaying` subscription per kiosk board through one `BoardPresenceProvider` each. It never resolves board ids itself — the `gymKiosk` query returns each board's presence-channel `boardId` pre-resolved (public boards only for anonymous viewers). Since #4408 that client is **login-less**: the default `KioskPresenceHub` the display routes mount connects with `authToken: null` and never touches `/api/internal/ws-auth` at all, and its presence client is read-only (writes reject with `KioskReadOnlyPresenceError`). The one authenticated consumer of the same hub is the gym-manage kiosk preview, which mounts `ViewerKioskPresenceHub` (`kiosk/presence/viewer-kiosk-presence-hub.tsx`) because `gymKiosk`'s edit branch hands it private gym boards that an anonymous read masks as `NOT_FOUND`. Kiosk reliability (5-minute manual catch-ups, config-poll reload, plus the hub's own client-rebuild supervisor — see [Client-Side Connection Supervisor](#client-side-connection-supervisor)) sits on top of the same reconnect/catch-up machinery described below. Since W-16 (#4435) the kiosk client is the only web graphql-ws consumer besides `social/comment-section.tsx` and `hooks/use-notification-subscription.ts` — the root `'session'` client that used to open a second socket on every route, kiosk TVs included, came out with `PersistentSessionWrapper`, so an anonymous kiosk TV now holds exactly one socket, an `/embed/board/…` page (which mounts the same hub) holds one, and `/embed/gym/…/leaderboard` holds none. A _signed-in_ viewer additionally holds the notification socket: `NotificationSubscriptionManager` is mounted at the root on every route (`app/layout.tsx`) and `use-notification-subscription.ts` opens its own client whenever there is an auth token — unchanged by W-16, and irrelevant to the kiosk/embed case, which is anonymous.
...
> **Two supervisors, two jobs — don't confuse them.** `WebSocketConnectionManager` supervises a _live but stale_ socket: it watches health events and calls `client.terminate()`, after which graphql-ws's own retry loop reconnects the same client. The kiosk hub supervises the two cases that outlive that loop, both of which end in the same move — dispose the client and build a fresh one, by bumping a `clientGeneration` effect dep:
>
> 1. **The socket is down and the retry budget is spent.** graphql-ws is configured with `retryAttempts: 10` and resets its `retries` counter only on a successful `connected`, so once the budget is gone the client is permanently dead and `terminate()` cannot revive it. `KioskPresenceHubInner` starts a `PRESENCE_REBUILD_AFTER_MS` (5 min, one notch above the full retry budget) countdown on `closed` and cancels it on `connected`.
> 2. **The socket is up but one board's subscription is dead.** graphql-ws does not retry an operation the server rejected with a GraphQL error (its subscribe loop treats `MessageType.Error` as terminal), and on a multi-board TV the other boards keep the socket alive — so no `closed` ever fires and nothing in the stack notices. The hub watches each board's `isLive` (published by `KioskBoardFeedBridge`) and runs the same countdown for a board that stays dead while the socket is connected. Capped at `MAX_STALE_SUBSCRIPTION_REBUILDS`, because a board flipped private mid-session is never coming back and would otherwise churn the client every five minutes until the 04:00 reload. The budget resets only after a full window with every board live. A socket outage preserves any spent subscription retries; replacing the transport does not establish that a rejected board recovered.
>
> **A rebuild costs the wall its state, and the hub has to pay it back explicitly.** A new client is a new client _identity_, so the shared `useBoardPresence` RESETs and re-runs its backfill seeds — while the backend is unreachable, which is the only reason case 1 fired. Those seeds are best-effort and swallow their failure, `createBoardPresenceClient` treats a fresh factory's first `connected` as the initial connect and skips its reconnect catch-up, and `boardNowPlaying` publishes no snapshot on subscribe. Left alone the screen comes back **permanently blank**. So the hub bumps a `catchUpNonce` on a rebuilt client's first `connected`; `KioskBoardFeedBridge` turns that into a `refresh('reconnect')`, and `BACKFILL_HISTORY` re-adopts the newest climb as `current`.
>
> Dispose runs before create (React effect cleanup ordering), so a kiosk still holds one socket — but only because the cleanup calls `terminate()` first when the socket is not up. `dispose()` alone awaits a `connecting` promise that only a `connection_ack` settles, and the shared client leaves `connectionAckWaitTimeout` at graphql-ws's default of `0`, which disables the ack timeout; a proxy that accepts the upgrade while the backend restarts would otherwise leave the disposed client's socket OPEN for the life of the page. `graphql-ws-wedged-dispose.test.ts` pins that upstream behaviour. The rebuild countdown carries the same jitter ratio as the transport's own backoff (`RECONNECT_JITTER_RATIO`), so a fleet that dropped together does not rebuild — and reconnect, since a freshly built client does not wait before its first attempt — in lockstep. The trade to know about: each generation gets a fresh 11-attempt budget (initial connect + `retryAttempts: 10`), so an outage that outlives the first window costs one extra burst per rebuild — with the jittered window averaging ~3.75 min, a 12-minute outage is on the order of 40 attempts rather than 11, spread over those 12 minutes and well inside `boardNowPlaying`'s 60/min anon bucket.
>
> This matters only on unattended screens: on a desk the equivalent fix is a page refresh. Note that `/embed/board/…` mounts the hub **without** `KioskReliability`, so an embed gets this supervisor and the per-board catch-up, but neither the config-poll reload nor the 04:00 reload.
