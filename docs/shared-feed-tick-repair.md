...
Both directions group by current and target board IDs, update at most 500 UUIDs per statement, and run all batches in one transaction. The current board must still match the snapshot, so a later re-file is skipped. Before a forward apply, a serializable transaction reloads every still-source tick, its active session wall, source feed configuration, and matching live owned boards. It rebuilds the destination with the same session-first rule; any changed or ambiguous destination aborts the whole apply before writes. Run a fresh dry-run and review its new snapshot after that refusal or a serialization failure; the script never retries an older plan or substitutes a different wall. Revert intentionally keeps only its historical current-board guard, because it restores the reviewed snapshot rather than resolving a new destination. Moved ticks receive a fresh sync timestamp; their climber and other tick fields are unchanged. The report distinguishes moves to session walls from sessions already correctly on the feed. Owner lookup reads are also bounded to 500 IDs. The script does not change board ownership.
