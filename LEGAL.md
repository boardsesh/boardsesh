...
**Moon Climbing** (MoonBoard): As of the last review of Moon Climbing's public-facing legal documents (February 2026), we did not find app-specific Terms of Service, an End User License Agreement, or a contributed-content license governing the MoonBoard app or its climb database — their website Terms and Conditions address e-commerce matters (shipping, returns, pricing) and their Privacy Policy addresses personal data under GDPR. As with the other boards, our position does not turn on any single manufacturer's terms: it rests on the factual, functional nature of climb definitions and the community-created nature of the databases (see above).
...
Boardsesh is open source under two licences. The product core (apps, backend and sync services, the Aura board renderer, product logic) is under the GNU Affero General Public License v3.0 or later (`LICENSE`), so that distributed or network-operated modifications remain available to their users. The interoperability infrastructure (API schema and clients, protocol definitions, board catalogue and adapters, Bluetooth libraries, controller firmware) is under the Apache License 2.0, so that integrations and independent hardware can build on it. [`LICENSING.md`](./LICENSING.md) is the authoritative description of the boundary; `REUSE.toml` is its machine-readable form.

The licences cover this implementation: its source, bindings, tuning data, traced geometry and compiled artifacts. They do not claim the ideas (dimming unlit holds, colouring holds by role, glowing a silhouette, rendering at any size, queueing climbs, talking to a board over Bluetooth), which anyone may implement independently. Releases before the transition remain under the Apache License 2.0, and contributions received under Apache-2.0 keep that licence inside the combined work. Separate commercial licensing of AGPL components could be available from the copyright holder; no terms are published here, and dual licensing, contributor permissions and official app-store distribution all require final legal review.
