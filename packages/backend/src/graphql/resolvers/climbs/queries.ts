...
import { isCanonicalStoredHold, parseFramesToHoldEntries, type NormalizedHold } from './climb-similarity';
  ...
  similarClimbs: async (
    ...
    if (validated.climbUuid) {
      const [climbRow] = await db
        .select({ frames: dbSchema.boardClimbs.frames, framesCount: dbSchema.boardClimbs.framesCount, compatibleSizeIds: dbSchema.boardClimbs.compatibleSizeIds })
