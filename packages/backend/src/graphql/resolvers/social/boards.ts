...
// This fragment requires the surrounding board_climb_holds alias `bch`.
// `hold_state` is NOT NULL in the schema, so excluding SQL NULL here is intentional.
export const POPULAR_CONFIGS_STRUCTURALLY_VALID_HOLD_PREDICATE = sql`(bch.hold_id > 0 OR (bch.hold_id = 0 AND bch.board_type = 'woods')) AND bch.hold_state <> '' AND bch.hold_state NOT LIKE '%=%'`;
...
export const POPULAR_CONFIGS_QUERY = sql`
  ...
  FROM (
    ...
    WHERE psls.is_listed = true
      -- A spray wall is private by default and never a popular board config.
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE bc.board_type = configs.board_type
      ...
      -- Match the list page: hidden climbs are not part of the advertised count.
