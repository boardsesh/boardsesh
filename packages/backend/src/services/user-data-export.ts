...
export async function buildUserAuroraJsonExport(userId: string, boardType: AuroraBoardName): Promise<AuroraJsonExport> {
  const [userRows, tickRows, favoriteRows, circuitRows, climbRows] = await Promise.all([
    ...
    dbRead
      ...
      .from(userFavorites)
      // Favorites are keyed by (user_id, climb_uuid); the board comes from the
      // catalog join, not from the favorite row.
      .innerJoin(boardClimbs, and(eq(userFavorites.climbUuid, boardClimbs.uuid), eq(boardClimbs.boardType, boardType)))
