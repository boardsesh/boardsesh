...
describe('searchGyms requireSlug', () => {
  it('pages claimed gyms first and fills remaining places with unclaimed gyms', async () => {
    ...
    expect(first.gyms.map((gym) => gym.uuid)).toEqual([claimedFirst.uuid, claimedSecond.uuid]);
    ...
    expect(defaultOrder.gyms.map((gym) => gym.uuid)).toEqual([unclaimed.uuid, claimedFirst.uuid, claimedSecond.uuid]);
  });
