...
describe('climb mutations', () => {
  ...
  describe('the spray write gate', () => {
    // `spray` is a real BoardName as of SW-03 (#5436) but ships with no
    // catalogue rows and no wall ownership, and the generic Aurora path does NOT
    // stop it on its own: populateDenormalizedColumns matches zero placements
    // for a spray layout and returns without throwing. So the gate is the only
    // thing between an authenticated caller and a listed board_climbs row in the
    // spray partition at a layoutId of their choosing — ids SW-04's per-wall
    // sequence would later hand to a real wall. Delete the guard and these two
    // go red. SW-05 (#5438) replaces it with the real spray branch.
    const SPRAY_SAVE_INPUT = {
      boardType: 'spray',
      layoutId: 900,
      name: 'Blue Traverse',
      description: '',
    ...
    it('refuses to create a climb on a spray wall', async () => {
      ...
      await expect(climbMutations.saveClimb({}, { input: SPRAY_SAVE_INPUT }, makeCtx())).rejects.toThrow(/spray wall/i);
      // Not "it errored somewhere later": nothing was written at all.
      expect(insertCalls).toHaveLength(0);
    });

    it('refuses to edit a climb on a spray wall', async () => {
      mockDb.execute.mockResolvedValue([]);
      ...
      ).rejects.toThrow(/spray wall/i);
      // The ownership/edit-window SELECT never ran — the gate is ahead of it.
      expect(mockDb.select).not.toHaveBeenCalled();
    });

    it('still lets a real board through the same code path', async () => {
      // Guards the guard: a gate that rejected everything would pass the two
      ...
      await climbMutations.saveClimb(
        {},
        { input: { ...SPRAY_SAVE_INPUT, boardType: 'kilter', frames: 'p1r43' } },
        makeCtx(),
      );
