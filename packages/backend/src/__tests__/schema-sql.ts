...
export const schemaSQL = `
  ...
  -- Hand-corrected hold silhouettes, overriding the traced geometry shard.
  -- Mirrors packages/db/src/schema/app/hold-outline-overrides.ts (migration
  -- 0207). The unique index is the upsert target, so it is not optional here.
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS "hold_outline_overrides_placement_idx"
    ON "hold_outline_overrides" ("board_name", "layout_id", "size_id", "placement_id");
