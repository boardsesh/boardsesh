...
describe('pinned-count pill', () => {
  ...
  it('sits outside the map so it cannot cover zoom controls or wrapping credits', () => {
    const { container } = render(
      <GymDirectoryMap pins={[pin(1, 51, -2)]} pinnedCount={15} shownCount={24} locale="en-US" />,
    );
    ...
    const mapRegion = container.querySelector('[data-testid="gym-directory-map"]');
    expect(mapRegion?.contains(pill)).toBe(false);
    expect(mapRegion?.nextElementSibling).toBe(pill);
  });
});
