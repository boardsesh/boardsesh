 ...
 *       response.
 *
 * The mobile global setup downloads and verifies the pinned snapshot first.
 * Missing fixtures fail the project setup instead of silently skipping coverage.
 *
...
import * as sharedLiveSessions from '@boardsesh/graphql/operations/live-sessions';
...
const FIXTURES_DIR = fixtureSnapshotDirectory();
...
const SHARED_OPERATION_MODULES: Record<string, Record<string, unknown>> = {
  ...
  '@boardsesh/graphql/operations/live-sessions': sharedLiveSessions,
...
function readManifest(): ScreenshotFixtureManifest {
  if (!existsSync(MANIFEST_PATH)) throw new Error(`Pinned screenshot fixture manifest is missing: ${MANIFEST_PATH}`);
...
describe('the recorded screenshot fixtures', () => {
  ...
  // time it is noticed, the name is already in the public snapshot.

  /** How many offending fields one failure line names before it says "+N more". */
  const PII_OFFENDERS_SHOWN = 5;

  it('carries real identity only for the recording account and approved test accounts', () => {
    const failures: string[] = [];
    for (const entry of entries) {
      const fixture = readFixture(entry.file);
