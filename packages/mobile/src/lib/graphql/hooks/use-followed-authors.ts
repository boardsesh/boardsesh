  ...
  return useMutation({
    ...
    mutationFn: async ({
      ...
      if (db && (await assertLocalUserDataOwner(db, userId)) === 'ok') {
        ...
        const removedUserIds = await writeAuthorFollowLocal(db, userId, kind, identifier, follow);
        ...
        return { queued: true, viewerId: userId, removedUserIds };
      } else {
        const document =
        ...
        return { queued: false, viewerId: userId, removedUserIds: [] as string[] };
      }
    },
    onSuccess: async ({ queued, viewerId, removedUserIds }, { kind }) => {
      if (removedUserIds.length > 0) {
        const { readLocalUserId } = await import('../../local-user-id');
        if ((await readLocalUserId()) !== viewerId) return;
        for (const removedUserId of removedUserIds) updateUserFollowCaches(queryClient, removedUserId, false, viewerId);
      }
