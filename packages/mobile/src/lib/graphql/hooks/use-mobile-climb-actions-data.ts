...
export function useMobileClimbActionsData(): MobileClimbActionsData {
  ...
  const toggleFavoriteMutation = useMutation({
    mutationFn: async (climbUuid: string): Promise<{ uuid: string; favorited: boolean }> => {
      const { queryClient: client, offlineEnabled: offline } = mutationDepsRef.current;
      // Favorites are keyed by (userId, climbUuid), so hearting a climb needs no
      // active board and no angle — it works from anywhere in the app.
      const input = { climbUuid };
