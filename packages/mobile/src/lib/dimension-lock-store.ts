...
// Callers fire this without awaiting, so a failed write is swallowed here rather
// than floating into error tracking; the in-memory lock still holds for the
// session, and the next successful write persists the whole bag.
async function persist(): Promise<void> {
  await setPreference(STORAGE_KEY, current).catch(() => {});
}
