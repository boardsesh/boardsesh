 ...
 * already up can never push a second copy of it, plus `qa`: the two gates run
 * side by side on a launch, and whichever finishes its reads first shows while
 * the other stands down on its route. Neither outranks the other. When this
 * notice is the one that yields, that costs nothing: the note is durable in the
 * database and the climber is told on the next launch instead.
 */
