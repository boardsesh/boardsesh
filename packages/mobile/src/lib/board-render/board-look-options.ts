 ...
 * only describes the Boardsesh drawing's knobs, and the classic marker overlay
...
export type BoardLookOption = {
   ...
   * step has no custom settings to show yet, so it previews the `bold` bundle
   * under a question mark instead.
   */
   ...
   * library that predates the Boardsesh drawing accepts its config, ignores
 ...
 *
 * The `boardsesh` half is inert: `buildBoardRenderSignature` returns `''` for a
...
const BOARDSESH_OPTION: BoardLookOption = {
  id: 'boardsesh',
...
const SUBTLE_OPTION: BoardLookOption = {
  ...
  descriptionI18nKey: 'mobile.more.boardLook.presets.descriptions.subtle',
...
const BOLD_OPTION: BoardLookOption = {
  ...
  descriptionI18nKey: 'mobile.more.boardLook.presets.descriptions.bold',
...
/**
 * The onboarding step's Custom card: a `bold` render under a question mark.
 ...
 *
 * Classic wins outright: every Boardsesh preset also carries a full `boardsesh`
 * bundle, so a climber on the classic drawing would otherwise match whichever
 * preset their untouched knobs happened to equal.
 *
 * `'default'` is normalised to `'boardsesh'` before matching. It means "the app
 * default", which since 2.4 IS the Boardsesh drawing — so a climber who has
 * never chosen a mode sits on whichever preset their knobs equal (the plain
 * `boardsesh` one, on a fresh install), not on `'custom'`. Without this the
...
export function matchingBoardLookOptionId(settings: BoardRenderSettings): BoardLookOptionId {
  ...
  return matchingPresetId(settings.mode === 'default' ? { ...settings, mode: 'boardsesh' } : settings);
}
 ...
 * drawing and back should find their Boardsesh knobs where they left them.
 * `'custom'` lands them on the plain Boardsesh bundle and the caller then opens
...
export async function applyBoardLookOption(id: BoardLookOptionId): Promise<void> {
  ...
  await applyBoardRenderPreset(id === 'custom' ? 'boardsesh' : id);
}
