...
describe('overlay-cache-store hydration + snapshot (warmup contract)', () => {
  ...
  it('flushes shared web entries when the native and web renderer contract moves to v16', async () => {
    const { store } = installCaches();
    await writeOverlayToCache('v2_s_wfull_kilter_1_2_25_old', new Blob() as Blob);
    ...
    // Drawn under an older generation — the stale Woods geometry v16 exists to evict.
    await writeOverlayToCache('v9_s_wfull_kilter_1_2_25_lit_plate', new Blob() as Blob);
    await writeOverlayToCache('v16_s_wfull_kilter_1_2_25_keep', new Blob() as Blob);
    await writeOverlayToCache('v1_f_w400_kilter_1_2_25_ancient', new Blob() as Blob);
    _overlayCacheStoreForTests.renderedObjectUrls.clear();

    expect(currentOverlayVersionPrefix()).toBe('v16_');
    ...
    expect(store.has(_overlayCacheStoreForTests.overlayKeyUrl('v16_s_wfull_kilter_1_2_25_keep'))).toBe(true);
    const entries = snapshotOverlayEntries();
    expect(entries).toHaveLength(1);
    expect(entries[0].name).toBe('v16_s_wfull_kilter_1_2_25_keep.png');
  });
