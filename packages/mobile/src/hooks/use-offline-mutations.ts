...
export async function writeAuthorFollowLocal(
  ...
  // Renamed helper, same reporting: this call site does NOT pass
  // `reviveDeadLetter`, so `revived` is always false here and the outcome
  // reported is exactly the suppression it always was. It surfaces a
  // dead-lettered key to the climber by throwing above instead of reviving it.
  reportEnqueueOutcome(table, operation, enqueueOutcome);
  notifyOutboxChanged();
