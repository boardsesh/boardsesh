...
describe('buildCacheKey', () => {
  ...
  it('uses RENDERER_VERSION v10 to invalidate overlays drawn before the LED base plate landed', () => {
    ...
    // binaries whose drawing was still moving. v10 drops the ones drawn before
    ...
    expect(buildCacheKey('kilter', 1, 10, '24', 'p1r42')).toMatch(/^v10_/);
  });
...
describe('renderedOverlays warm-up from disk cache', () => {
  ...
  it('exposes the populated map so a fresh hook init can hit it synchronously', () => {
    ...
    _cacheRenderedOverlayForTests('v10_kilter_1_10_24_deadbeef', 'file:///prior/session.png');
    expect(_renderedOverlaysForTests.get('v10_kilter_1_10_24_deadbeef')?.uri).toBe('file:///prior/session.png');
    _renderedOverlaysForTests.delete('v10_kilter_1_10_24_deadbeef');
  });
  ...
  it('only loads PNGs whose name starts with the current RENDERER_VERSION prefix', () => {
    ...
    // v10 entries. The warm-up must surface only v10 keys; every older file is
    // invalid.
    ...
    // are on real devices and are the whole reason v10 exists.
    const v9Entry = makeMockEntry('v9_kilter_1_10_24_litplate.png');
    const v10EntryA = makeMockEntry('v10_kilter_1_10_24_cccccccc.png');
    const v10EntryB = makeMockEntry('v10_tension_2_8_15_dddddddd.png');
    ...
    expect(_renderedOverlaysForTests.has('v10_kilter_1_10_24_cccccccc')).toBe(true);
    expect(_renderedOverlaysForTests.has('v10_tension_2_8_15_dddddddd')).toBe(true);
    ...
    // Only the two v10 entries should be present — no stragglers from
  ...
  it('opportunistically deletes stale-version PNG files to reclaim disk', () => {
    ...
    const v9Entry = makeMockEntry('v9_kilter_1_10_24_litplate.png');
    const v10Entry = makeMockEntry('v10_kilter_1_10_24_cccccccc.png');
    directoryListSpy.mockReturnValue([
      v1Entry,
      v2Entry,
      v4Entry,
  ...
  it('keeps loading remaining entries when a delete throws', () => {
    ...
    const v10Entry = makeMockEntry('v10_kilter_1_10_24_bbbbbbbb.png');
    directoryListSpy.mockReturnValue([v1Entry, v10Entry]);

    expect(() => _runWarmupForTests()).not.toThrow();
    expect(_renderedOverlaysForTests.has('v10_kilter_1_10_24_bbbbbbbb')).toBe(true);
  });
});
