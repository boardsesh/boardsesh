...
// out of BoardSheet so it can render either inside the gorhom BottomSheetModal
// (variant 'sheet', using BottomSheetFlatList) or as a standalone inline column
// (variant 'column', using a plain react-native FlatList) for the iPad shell.
//
// Owns the action-resolution machinery (GET_CLIMB hydration + LRU cache, pending
// generation guards) and the interactive row handlers. The hero, stat tiles,
...
export type NowOnTheWallPanelProps = {
  /** 'sheet' renders inside the gorhom modal (BottomSheetFlatList); 'column' renders inline (FlatList). */
