...
/**
 * Gate 5's residue, pinned as the honest current state rather than as zero.
 *
 * `trimThinNecks` runs on the raster mask; the emitted polygon is that mask's
 * border after Douglas-Peucker at 1.6 px, which can shave up to that much off
 * each side of a neck. Two Kilter Homewall limbs sat just wide enough to keep a
 * core in the mask and come back under it in the polygon. Neither placement is
 * lit by the spike's synthesised climb, so neither has ever been painted; the
 * fix, if one is wanted, is to run the open before the simplify. Anything
 * beyond these two is a regression.
 */
const KNOWN_SPUR_HOLD_IDS: Record<string, number[]> = {
  'kilter-homewall-10x12': [4135, 4634],
};
...
type Raster = { filled: Uint8Array; width: number; height: number; left: number; top: number; area: number };
...
function rasterise(flat: number[]): Raster {
  ...
  return { filled, width, height, left, top, area };
}
...
/**
 * Board px² an open at `NECK_TRIM_RADIUS` cuts away from the polygon — the spur
 * measure design review 2's change 2 asked for, and not a perimeter one: a
 ...
 *
 * A mask with no core comes back untouched, matching the generator: MoonBoard
 * 2016's hold 148 is a 12x35 rail that an unconditional open would delete.
 */
function spurArea(flat: number[]): number {
  const { filled, width, height, left, top, area } = rasterise(flat);
  ...
  const grown = new Uint8Array(filled.length);
  for (let index = 0; index < core.length; index += 1) {
    ...
    for (const [dx, dy] of NECK_DILATION_OFFSETS) {
      ...
      if (filled[ny * width + nx] === 1) grown[ny * width + nx] = 1;
    }
  }

  // Flood from the placement, which gate 1 has already established is inside.
  const seed = -top * width + -left;
  if (grown[seed] !== 1) return 0;
  const kept = new Uint8Array(grown.length);
  const stack = [seed];
  kept[seed] = 1;
  let keptArea = 0;
  while (stack.length > 0) {
    ...
    for (const [dx, dy] of ORTHOGONAL) {
      ...
      if (kept[neighbour] === 1 || grown[neighbour] !== 1) continue;
      kept[neighbour] = 1;
      stack.push(neighbour);
    }
  }
  return area - keptArea;
}
...
describe('SPIKE_HOLD_OUTLINES gates', () => {
  ...
  it('gate 5: no outline loses more than 20 board px² to a thin-necked limb', () => {
    for (const audit of AUDITS) {
      expect([audit.boardKey, audit.spurred]).toEqual([audit.boardKey, KNOWN_SPUR_HOLD_IDS[audit.boardKey] ?? []]);
    }
  });
});
