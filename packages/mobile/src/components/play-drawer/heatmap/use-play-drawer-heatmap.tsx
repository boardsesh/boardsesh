...
import { HeatmapOverlay, type HeatmapMode } from '../../board/HeatmapOverlay';
...
export type PlayDrawerHeatmap = {
  ...
  isResolving: boolean;
  /** The saved search needs a filter this phone cannot run (hold states). */
 ...
 * answer comes from (`useCatalogQuerySource`), and the memoised overlay.
 *
...
export function usePlayDrawerHeatmap(board: PlayDrawerHeatmapBoard): PlayDrawerHeatmap {
  ...
  const [mode, setMode] = useState<HeatmapMode>('uses');
  ...
  const filterUnsupported = source === 'local' && !isOfflineSearchSupported(input);
  ...
  const overlay = useMemo(
    () =>
      showOverlay && holds ? (
        <HeatmapOverlay
          statsByHoldId={heatmap.statsByHoldId}
    ...
    [showOverlay, holds, heatmap.statsByHoldId, mode],
  );
