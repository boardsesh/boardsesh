...
type ViewProps = { children?: ReactNode; testID?: string; onLayout?: LayoutHandler };

// vi.mock factories are hoisted above module scope, so anything they close over
// has to be hoisted with them.
const { renderView, zoomState, recorded } = vi.hoisted(() => {
  const recorded: { onLayout?: LayoutHandler } = {};
  return {
    recorded,
    renderView: ({ children, testID, onLayout }: ViewProps) => {
      // The carousel bails out to a bare measuring container until onLayout has
      // reported a box, so a test has to hand it one before anything else mounts.
...
// Drawn siblings of the overlay; neither carries a gesture, so they stay stubs.
vi.mock('../../board-controls/ResetZoomButton', () => ({ ResetZoomButton: () => null }));
vi.mock('../UpcomingBoardPrefetch', () => ({ UpcomingBoardPrefetch: () => null }));
...
/** Report a measured board box, the way RN would once the flex box has laid out. */
function measure(): void {
  const onLayout = recorded.onLayout;
  if (!onLayout) throw new Error('Carousel measurement container is missing');
  act(() => onLayout({ nativeEvent: { layout: { width: 390, height: 600 } } }));
}
