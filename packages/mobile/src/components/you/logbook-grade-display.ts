 ...
 * it). The community consensus is surfaced as a small `people`-marked secondary
 * only when it disagrees with the logged grade, and an ungraded row's grade is
 * marked as consensus-sourced so it's clear it's the crowd's, not the climber's.
 *
 * Pure difficulty-id comparison so the row's display branches stay unit-testable;
 * the grade-label formatting (which needs the grade-format hook) lives in the row.
 */
