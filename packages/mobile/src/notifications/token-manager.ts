...
function jitterRetryDelay(delayMs: number): number {
  if (delayMs === 0) return 0;
  const factor = 1 - RETRY_JITTER_RATIO + Math.random() * RETRY_JITTER_RATIO * 2;
...
async function registerWithRetry(register: TokenRegistrationFn, sessionId: string, token: string): Promise<boolean> {
  for (let attempt = 0; attempt < RETRY_DELAYS.length; attempt++) {
    const delay = jitterRetryDelay(RETRY_DELAYS[attempt]);
    if (delay > 0) {
