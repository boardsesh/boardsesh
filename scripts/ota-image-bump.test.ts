...
import { parseTestPlan, parseRisk } from '../packages/shared/pr-body/src/index';
...
describe('pullRequestBody', () => {
  it('keeps pre-merge QA executable and moves operator checks after the merge', () => {
    const body = pullRequestBody('3.1.3', '3.1.2');
    expect(parseTestPlan(body)?.steps).toEqual(['CI green.']);
    expect(parseRisk(body)?.level).toBe(5);
    const afterMerge = body.split('## After it lands')[1]?.split('## Release Notes')[0];
    expect(afterMerge).toContain('Railway Config');
    expect(afterMerge).toContain('https://updates.boardsesh.com/hc');
  });
  ...
  it('asks the reader to check readiness, not ClickHouse system log TTLs', () => {
    // The lean ClickHouse image disables the system logs in config, so an xprem bump
    // cannot bring them back; readiness is what proves xprem still reaches ClickHouse.
    const body = pullRequestBody('3.1.3', '3.1.2');
    expect(body).toContain('https://updates.boardsesh.com/ready');
    expect(body).not.toContain('system.*_log');
  });
});
