...
void test('resolves each single target', () => {
  ...
  assert.deepEqual(resolveWebDeployTargets({ raw: 'railway', railwayWebServiceId: SERVICE_ID }), {
    vercel: false,
    railway: true,
    targets: 'railway',
  });
...
void test('accepts both targets in either order, with whitespace and casing tolerated', () => {
  ...
  for (const raw of ['vercel,railway', 'railway,vercel', ' vercel , railway ', 'Vercel,RAILWAY', 'vercel,\trailway']) {
    assert.deepEqual(resolveWebDeployTargets({ raw, railwayWebServiceId: SERVICE_ID }), expected, raw);
  }
});
...
void test('rejects "none" mixed with a real target', () => {
  ...
  assert.throws(
    () => resolveWebDeployTargets({ raw: 'railway,none', railwayWebServiceId: SERVICE_ID }),
    /cannot be combined/,
  );
});
