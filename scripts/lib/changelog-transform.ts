...
// a footer that sits right after the notes — with no heading to bound it — from
...
const SECTION_BREAK = /^\s*(?:(?:-{3,}|\*{3,}|_{3,})\s*$|🤖|[_*]generated (?:by|with)\s)/i;
...
// Release Notes section with nothing to bound them (see PR #3367, where a
// Co-Authored-By trailer shipped as a What's New card title). Never release copy,
// so drop the line wherever it appears. Deliberately NOT a generic `word:` match —
// prose like "Faster search: results in half the time" must survive.
const GIT_TRAILER = /^(?:[a-z][a-z-]*-by|change-id):\s/i;
// A line that is nothing but a URL (e.g. a bare claude.ai/code session link, see
// PR #3180). A URL inside a sentence is kept; a link-only line is never copy a
// climber should read.
 ...
 * code blocks. HTML comments and bullet markers are stripped; standalone `none`
 * lines (case-insensitive), git trailers (`Co-Authored-By:` and friends), and
 * bare link-only URL lines are dropped. Returns null when nothing is left — those
 * PRs (absent / empty / `none` / trailer-only) produce no changelog entry.
 */
export function extractReleaseNotes(prBody: string | null | undefined): ReleaseNotes | null {
  ...
  // never content), then blank lines, bullet markers, any standalone `none`, git
  // trailers, and link-only URL lines, so none of them ship as a junk entry.
  const cleaned = sectionLines
    .filter((line) => !NO_RELEASE_NOTE_BOX.test(line))
    .map((line) => line.replace(LIST_MARKER, '').trim())
    .filter(
      (line) => line.length > 0 && !NONE_MARKER.test(line) && !GIT_TRAILER.test(line) && !BARE_URL_LINE.test(line),
    );
