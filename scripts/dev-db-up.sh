...
detect_local_redis_url() {
  redis_container=$(docker ps --filter "publish=6379" --filter "status=running" -q 2>/dev/null | sed -n '1p')
...
run_pending_drizzle_sql_migrations() {
  ...
  # The JavaScript program is intentionally single-quoted so Bash does not
  # expand its template literals.
...
if docker_daemon_available; then
  ...
  PG_CONTAINER=$(docker ps --filter "publish=5432" --filter "status=running" -q 2>/dev/null | sed -n '1p')
