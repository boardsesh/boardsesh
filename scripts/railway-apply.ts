...
const SERVICE_INSTANCE_QUERY = `
  query ServiceInstanceForApply($environmentId: String!, $serviceId: String!) {
    serviceInstance(environmentId: $environmentId, serviceId: $serviceId) {
      ...
      activeDeployments { id status meta canRollback }
    }
  }
`;
...
async function fetchVariables(
  ...
  for (const declared of desired.services) {
    ...
    const hasVariableAssertions =
      ...
      (declared.forbiddenVars?.length ?? 0) > 0 ||
...
interface InstanceRead {
  ...
  servingDeployment: DeploymentSnapshot | null;
...
async function fetchServiceInstances(
...
): Promise<Map<string, InstanceRead | null>> {
  const reads = new Map<string, InstanceRead | null>();
  ...
  for (const name of servicesNeedingInstanceRead(desired)) {
    ...
    const activeDeployments = raw.activeDeployments ?? [];
    const successfulDeployments = activeDeployments.filter((deployment) => deployment.status === 'SUCCESS');
    // latestDeployment can be a failed/canceled attempt while a prior SUCCESS
    // still serves traffic. Never use that attempt as the running image or
    // rollback baseline, and never guess between multiple serving identities.
    const serving = successfulDeployments.length === 1 ? successfulDeployments[0] : null;
    const servingDeployment: DeploymentSnapshot | null = serving
      ? {
          id: serving.id,
          status: serving.status,
          image: deploymentImage(serving.meta),
          canRollback: serving.canRollback,
        }
      : null;
    ...
    reads.set(name, {
      instance: {
        ...
        runningImage: servingDeployment?.image ?? null,
...
export async function main(
  ...
  const live: LiveState = {
    ...
    instances: Object.fromEntries([...instanceReads].map(([name, read]) => [name, read?.instance ?? null])),
    clickhouseTtl,
  ...
  if (blockedServiceWrites) {
    console.warn(
      '[railway-apply] No changes applied: a service with blocked image drift also has writable drift. ' +
        'Resolve the blocked image drift before retrying so one verified deployment carries the complete configuration.',
    );
    return 1;
  }
  ...
  if (mutations.size > 0) {
    ...
    // Keep the existing single-service best-effort policy when there is no prior
    // rollback target. Multi-service batches require every target before writes
    // so a later failure cannot strand an already-converged earlier service.
    if (options.wait && mutations.size > 1) {
      const withoutRollbackTarget = [...mutations.keys()].filter(
        (serviceName) => !instanceReads.get(serviceName)?.servingDeployment?.canRollback,
      );
  ...
  for (const mutation of mutations.values()) {
    ...
    const previousDeployment = read?.servingDeployment ?? null;
