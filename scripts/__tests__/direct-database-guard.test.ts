...
describe('direct database guard', () => {
  ...
  it('rejects port zero while accepting valid bracketed IPv6 endpoint identities', () => {
    expect(() => databaseEndpointIdentity('postgresql://migrator:secret@direct.example:0/boardsesh')).toThrow(
      'port must be between 1 and 65535',
    );
    expect(() =>
      assertExpectedDirectEndpoint('postgresql://migrator:secret@[::1]:5432/boardsesh', '[::1]:5432/boardsesh'),
    ).not.toThrow();
  });
  ...
  it('redacts malformed connection strings from parse errors', () => {
    ...
    expect(String(thrownError)).toContain('MIGRATOR_DATABASE_URL is not a valid PostgreSQL URL');
  });
  ...
  it('runs a read-only connectivity probe and redacts failures', async () => {
    ...
    ).rejects.toThrow('MIGRATOR_DATABASE_URL failed TLS connectivity or SELECT 1 verification');
  });
});
