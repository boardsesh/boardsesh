...
describe('extractReleaseNotes', () => {
  ...
  it('drops git trailers left after the notes (PR #3367 regression)', () => {
    // The exact shape that shipped "Co-Authored-By: …" as a What's New card:
    // `none`, then an unbounded trailer + session link. Must produce no entry.
    const trailerAfterNone = [
      '## Release Notes',
      '',
      'none',
      '',
    ...
    expect(extractReleaseNotes(trailerAfterNone)).toBeNull();

    // A real note survives; the trailer block after it is stripped.
    const trailerAfterNote = [
      '## Release Notes',
      'Queue sync is faster',
      '',
      'Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>',
      'Signed-off-by: Someone <someone@example.com>',
      'Reviewed-by: Someone Else <else@example.com>',
      'Co-developed-by: A Third <third@example.com>',
      'Change-Id: I8f2a9c4e',
    ].join('\n');
    expect(extractReleaseNotes(trailerAfterNote)).toEqual({ title: 'Queue sync is faster' });
  });

  it('drops bare link-only URL lines (PR #3180 regression)', () => {
    // A session link as the only remaining content must not become a card title.
    expect(extractReleaseNotes('## Release Notes\n\nnone\n\nhttps://claude.ai/code/session_0169kjQT2Bwq')).toBeNull();
    // Bulleted bare links are just as junk.
    expect(extractReleaseNotes('## Release Notes\n- https://claude.ai/code/session_x')).toBeNull();
    // But a URL inside a sentence is real copy and stays.
    expect(extractReleaseNotes('## Release Notes\nShare boards via https://boardsesh.com links')).toEqual({
      title: 'Share boards via https://boardsesh.com links',
    });
  });

  it('keeps prose that merely contains a colon (not a git trailer)', () => {
    expect(extractReleaseNotes('## Release Notes\nFaster search: results in half the time')).toEqual({
      title: 'Faster search: results in half the time',
    });
    expect(extractReleaseNotes('## Release Notes\nNew: reorder the queue by dragging')).toEqual({
      title: 'New: reorder the queue by dragging',
    });
  });

  it('stops the section at the "_Generated by [Claude Code](…)_" footer variant', () => {
    // The claude.ai/code footer without a `---` before it must still be excluded.
    const body =
      '## Release Notes\nQueue sync is faster\n\n_Generated by [Claude Code](https://claude.ai/code/session_x)_\nNot notes';
    expect(extractReleaseNotes(body)).toEqual({ title: 'Queue sync is faster' });
    expect(extractReleaseNotes('## Release Notes\nnone\n\n_Generated by [Claude Code](x)_')).toBeNull();
    // Asterisk-italic footer variant is bounded the same way.
    expect(extractReleaseNotes('## Release Notes\nnone\n\n*Generated by [Claude Code](x)*')).toBeNull();
    // But real prose starting with "Generated by" (no italic delimiter) is kept,
    // including as a bullet (the `- `/`* ` marker has a space, unlike the footer).
    expect(extractReleaseNotes('## Release Notes\nGenerated by popular demand: dark mode')).toEqual({
      title: 'Generated by popular demand: dark mode',
    });
    expect(extractReleaseNotes('## Release Notes\n* Generated share links for playlists')).toEqual({
      title: 'Generated share links for playlists',
    });
  });
});
