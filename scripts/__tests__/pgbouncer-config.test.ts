...
describe('PgBouncer image publication contract', () => {
  ...
  it('verifies the same dedicated migration URL while retaining owner and runtime ACL gates', () => {
    const workflow = parse(readFileSync(PRODUCTION_WORKFLOW_PATH, 'utf8')) as {
      jobs: { migrate: { steps: { name?: string; env?: Record<string, string>; run?: string }[] } };
    };
    const steps = workflow.jobs.migrate.steps;
    const guard = steps.find((step) => step.name === 'Verify direct database connection');
    const migration = steps.find((step) => step.name === 'Run database migrations');
    expect(guard).toBeDefined();
    expect(migration).toBeDefined();
    expect(steps.indexOf(guard!)).toBeLessThan(steps.indexOf(migration!));
    ...
    expect(guard?.run).toBe('vp exec tsx packages/db/scripts/verify-direct-database.ts');
    expect(migration?.env).toMatchObject({
      DATABASE_URL: guard?.env?.MIGRATOR_DATABASE_URL,
    ...
    expect(migration?.env?.DATABASE_URL).toBe('${{ secrets.MIGRATOR_DATABASE_URL }}');
  });
});
