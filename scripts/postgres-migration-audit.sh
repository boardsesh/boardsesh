...
MIGRATION_SLOT_NAME="${MIGRATION_SLOT_NAME:-$MIGRATION_SUBSCRIPTION_NAME}"
...
MIGRATION_RUNTIME_SCHEMAS="${MIGRATION_RUNTIME_SCHEMAS:-public}"
...
Optional target comparison:
  ...
  MIGRATION_REQUIRED_ROLES         Optional additional role names to require.
                                    The four named role variables are required when
                                    TARGET_DATABASE_URL is set.
  MIGRATION_RUNTIME_SCHEMAS        Schemas the runtime role must access (default public).
...
Optional controls:
  ...
  MIGRATION_SLOT_NAME              Simple identifier (defaults to subscription name).
...
psql_readonly() {
  ...
  PGCONNECT_TIMEOUT="${PGCONNECT_TIMEOUT:-10}" \
    PGOPTIONS="${PGOPTIONS:+$PGOPTIONS }-c default_transaction_read_only=on -c statement_timeout=30000" \
...
psql_publisher_readonly() {
  PGCONNECT_TIMEOUT="${PGCONNECT_TIMEOUT:-10}" \
    PGOPTIONS="${PGOPTIONS:+$PGOPTIONS }-c default_transaction_read_only=on -c statement_timeout=30000" \
    psql "$SOURCE_REPLICATION_DATABASE_URL" -X -v ON_ERROR_STOP=1 "$@"
}
...
WITH fingerprint_rows AS (
  ...
  SELECT 'index',
         ...
         pg_catalog.concat_ws('|', index_row.indisunique::text, index_row.indisprimary::text,
           index_row.indisvalid::text, pg_catalog.pg_get_indexdef(index_row.indexrelid))
  ...
  SELECT 'type',
         ...
         pg_catalog.concat_ws('|', type.typtype, pg_catalog.format_type(type.typbasetype, type.typtypmod),
           coalesce(enum_values.enum_labels, ''))
  FROM pg_catalog.pg_type AS type
  JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = type.typnamespace
  LEFT JOIN LATERAL (
    SELECT pg_catalog.string_agg(enum.enumlabel, ',' ORDER BY enum.enumsortorder) AS enum_labels
...
unknown_schemas="$(scalar "$SOURCE_DATABASE_URL" "
SELECT coalesce(pg_catalog.string_agg(DISTINCT namespace.nspname, ', ' ORDER BY namespace.nspname), '')
FROM pg_catalog.pg_class AS relation
JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = relation.relnamespace
WHERE relation.relkind IN ('r', 'p', 'S', 'm', 'f')
  AND relation.relpersistence = 'p'
  AND namespace.nspname NOT IN (${included_schemas_sql})
  AND namespace.nspname NOT IN (${excluded_schemas_sql})
...
[[ -z "$unknown_schemas" ]] ||
  blocker "persistent non-extension relations exist in unclassified schema(s): $unknown_schemas; add each schema to the explicit include or exclude policy"
...
WHERE pubname = '${MIGRATION_PUBLICATION_NAME}';")"
    [[ "$publication_contract" == 't' ]] ||
      blocker "publication must include insert/update/delete/truncate and set publish_via_partition_root=false"
...
WHERE rolname = current_user;")"
    [[ "$publisher_contract" == 'true|false|true|false' ]] ||
      blocker "publisher role $publisher_role must be LOGIN, NOSUPERUSER, REPLICATION, and NOBYPASSRLS"
...
WHERE relation.relkind IN ('r', 'p')
  ...
  AND namespace.nspname IN (${included_schemas_sql})
  AND namespace.nspname NOT IN (${excluded_schemas_sql})
  AND NOT pg_catalog.has_table_privilege(current_user, relation.oid, 'SELECT')
  AND NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_depend AS extension_dependency
    WHERE extension_dependency.classid = 'pg_catalog.pg_class'::pg_catalog.regclass
      AND extension_dependency.objid = relation.oid
    ...
    [[ "$publisher_select_blockers" == '0' ]] ||
      blocker "publisher role $publisher_role lacks SELECT on $publisher_select_blockers published table(s)"
...
if [[ -n "$TARGET_DATABASE_URL" ]]; then
  ...
  target_unknown_schemas="$(scalar "$TARGET_DATABASE_URL" "
SELECT coalesce(pg_catalog.string_agg(DISTINCT namespace.nspname, ', ' ORDER BY namespace.nspname), '')
FROM pg_catalog.pg_class AS relation
JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = relation.relnamespace
WHERE relation.relkind IN ('r', 'p', 'S', 'm', 'f')
  ...
  [[ -z "$target_unknown_schemas" ]] ||
    blocker "target has persistent non-extension relations in unclassified schema(s): $target_unknown_schemas"
  ...
  if [[ -s "$missing_target_extensions_file" ]]; then
    blocker "target is missing source extension(s): $(paste -sd, "$missing_target_extensions_file" | sed 's/,/, /g')"
  fi
...
WHERE subscription.subname = '${MIGRATION_SUBSCRIPTION_NAME}'
  ...
  AND subscription.subpublications = ARRAY['${MIGRATION_PUBLICATION_NAME}']::text[];")"
      [[ "$subscription_contract" == 't' ]] ||
        blocker "target subscription owner/publication/slot/enabled/binary/origin/run_as_owner/password/failover contract differs"
    fi
...
FROM pg_catalog.pg_roles WHERE rolname = '${MIGRATION_REPLICATION_ROLE}';")"
    [[ "$owner_role_contract" == 'false|false|false|false' ]] ||
      blocker "owner role must be NOLOGIN, NOSUPERUSER, NOREPLICATION, and NOBYPASSRLS"
    owner_database_create="$(scalar "$TARGET_DATABASE_URL" "
SELECT pg_catalog.has_database_privilege(oid, current_database(), 'CREATE')
FROM pg_catalog.pg_roles
WHERE rolname = '${MIGRATION_OWNER_ROLE}';")"
    [[ "$owner_database_create" == 't' ]] ||
      blocker "owner role lacks CREATE on the target database and cannot restore/create application schemas"
    [[ "$runtime_role_contract" == 'true|false|false|false' ]] ||
      blocker "runtime role must be LOGIN, NOSUPERUSER, NOREPLICATION, and NOBYPASSRLS"
    [[ "$migrator_role_contract" == 'true|false|false|false' ]] ||
      blocker "migrator role must be LOGIN, NOSUPERUSER, NOREPLICATION, and NOBYPASSRLS"
    [[ "$replication_role_contract" == 'true|false|true|false' ]] ||
      blocker "replication role must be LOGIN, NOSUPERUSER, REPLICATION, and NOBYPASSRLS"
...
WHERE owner_role.rolname = '${MIGRATION_OWNER_ROLE}'
  AND member_role.rolname = '${MIGRATION_MIGRATOR_ROLE}'
...
SELECT rolcanlogin::text || '|' || rolsuper::text || '|' || rolreplication::text || '|' || rolbypassrls::text
FROM pg_catalog.pg_roles WHERE rolname = '${MIGRATION_SUBSCRIBER_ROLE}';")"
      [[ "$subscriber_role_contract" == 'true|false|false|false' ]] ||
        blocker "subscriber owner must be LOGIN, NOSUPERUSER, NOREPLICATION, and NOBYPASSRLS"

      subscriber_can_set_owner="$(scalar "$TARGET_DATABASE_URL" "
SELECT count(*) = 1
FROM pg_catalog.pg_auth_members AS membership
JOIN pg_catalog.pg_roles AS owner_role ON owner_role.oid = membership.roleid
JOIN pg_catalog.pg_roles AS subscriber_role ON subscriber_role.oid = membership.member
WHERE owner_role.rolname = '${MIGRATION_OWNER_ROLE}'
  AND subscriber_role.rolname = '${MIGRATION_SUBSCRIBER_ROLE}'
  AND membership.set_option
  AND NOT membership.inherit_option;")"
...
SELECT pg_catalog.has_database_privilege(subscriber.oid, current_database(), 'CREATE')
       AND pg_catalog.pg_has_role(subscriber.oid, create_subscription_role.oid, 'USAGE')
...
WHERE subscriber.rolname = '${MIGRATION_SUBSCRIBER_ROLE}'
  AND create_subscription_role.rolname = 'pg_create_subscription';")"
      [[ "$subscriber_create_contract" == 't' ]] ||
        blocker "subscriber owner lacks database CREATE or effective pg_create_subscription membership"
...
WHERE namespace.nspname IN (${included_schemas_sql})
  ...
  AND (owner.rolname <> '${MIGRATION_OWNER_ROLE}' OR owner.rolsuper)
  AND NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_depend AS extension_dependency
    WHERE extension_dependency.classid = 'pg_catalog.pg_proc'::pg_catalog.regclass
      AND extension_dependency.objid = procedure.oid
...
WHERE relation.relkind IN ('r', 'p')
  ...
  AND NOT (
    pg_catalog.has_table_privilege((SELECT oid FROM pg_catalog.pg_roles WHERE rolname = '${MIGRATION_RUNTIME_ROLE}'), relation.oid, 'SELECT')
    AND pg_catalog.has_table_privilege((SELECT oid FROM pg_catalog.pg_roles WHERE rolname = '${MIGRATION_RUNTIME_ROLE}'), relation.oid, 'INSERT')
    AND pg_catalog.has_table_privilege((SELECT oid FROM pg_catalog.pg_roles WHERE rolname = '${MIGRATION_RUNTIME_ROLE}'), relation.oid, 'UPDATE')
    AND pg_catalog.has_table_privilege((SELECT oid FROM pg_catalog.pg_roles WHERE rolname = '${MIGRATION_RUNTIME_ROLE}'), relation.oid, 'DELETE')
  )
  AND NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_depend AS extension_dependency
    WHERE extension_dependency.classid = 'pg_catalog.pg_class'::pg_catalog.regclass
    ...
    [[ "$runtime_table_blockers" == '0' ]] ||
      blocker "runtime role lacks CRUD privileges on $runtime_table_blockers required table(s)"
...
WHERE namespace.nspname IN (${runtime_schemas_sql})
  ...
  AND NOT EXISTS (
    ...
    [[ "$runtime_function_blockers" == '0' ]] ||
      blocker "runtime role lacks EXECUTE on $runtime_function_blockers required function(s)"
...
WHERE namespace.nspname IN (${runtime_schemas_sql})
  ...
  AND NOT EXISTS (
    ...
    [[ "$runtime_type_blockers" == '0' ]] ||
      blocker "runtime role lacks USAGE on $runtime_type_blockers required user-defined type(s)"
...
WHERE NOT EXISTS (
  ...
  WHERE default_acl.defaclrole = owner_role.oid
    AND default_acl.defaclnamespace IN (0, runtime_schema.oid)
    ...
    [[ "$default_privilege_blockers" == '0' ]] ||
      blocker "$default_privilege_blockers required owner→runtime default privilege(s) are missing for future tables, sequences, or functions"
