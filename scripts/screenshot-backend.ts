...
async function main(): Promise<void> {
  ...
  if (options.mode === 'replay' && options.fixturesDir === resolve(REPO_ROOT, DEFAULT_FIXTURES_DIR)) {
    try {
      options.fixturesDir = await ensureScreenshotFixtures();
    } catch (error) {
      fail(error instanceof Error ? error.message : 'Could not download pinned screenshot fixtures');
    }
  }
