...
The app icon and splash screen are defined in `packages/mobile/app.config.ts` (`icon: ./assets/icon.png` and the `expo-splash-screen` plugin with `splash-icon.png`). The native Android resources (mipmap and drawable directories, asset catalogs) are generated by `expo prebuild` from that config — they are not committed and there is no separate asset-generation step.
...
For a quick local AAB without EAS, generate the Android project and build with
Gradle:
...
Go to **Grow > Store presence > Main store listing**. The listing text (short description, full description, release notes) is source-controlled in `fastlane/metadata/android/<locale>/` and uploaded by the fastlane `android metadata` lane; see the operational doc `app-stores/google/play-store-metadata.md`. Fill in each field using those values:
...
| Asset           | Spec                                 | Notes                                                            |
| --------------- | ------------------------------------ | ---------------------------------------------------------------- |
| App icon        | 512x512 PNG, 32-bit, no transparency | Derived from `packages/mobile/assets/icon.png`                   |
...
4. Fill in the **Release notes** (What's New). Use the text from `fastlane/metadata/android/<locale>/` (see `app-stores/google/play-store-metadata.md`).
...
Google may flag apps that are primarily WebView wrappers. Boardsesh is not one. It is a native Expo React Native app that renders native Android UI, with no WebView wrapper around a website. The defense:

- Boardsesh is a native React Native (Expo) app. It does not load `boardsesh.com` in a WebView; the screens are native components.
- It uses native Android Bluetooth APIs (via `react-native-ble-plx`, which bridges to native Android Bluetooth) to communicate with climbing board hardware over BLE. This provides better connection reliability, persistent connections that survive screen-off, no Chrome permission dialogs, background BLE support so the board stays lit while the user climbs, and native app lifecycle management.
- The app also provides native deep link handling (`/join/*` Android App Links) and a native splash screen.
- If questioned, respond with: "Boardsesh is a native React Native app, not a WebView wrapper. It uses native Android BLE (react-native-ble-plx) to scan for climbing boards advertising the Aurora service (UUID 4488b571-7806-4df6-bcff-a2897e4953ff) and writes LED lighting commands to the Nordic UART RX characteristic (UUID 6e400002-b5a3-f393-e0a9-e50e24dcca9e). The native BLE layer provides persistent connections, background support, and better reliability than Web Bluetooth. The app also handles Android App Links natively for session deep linking."

### Device and Feature Targeting

The app requires BLE but does not currently declare a `<uses-feature>` for it. The native `AndroidManifest.xml` is generated by `expo prebuild`, so add this through `packages/mobile/app.config.ts` (a manifest config plugin / the `android` block) rather than editing the generated manifest directly, which gets overwritten on the next prebuild. The generated manifest should end up with:
