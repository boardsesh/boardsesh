 ...
 *
 * Or via the repo task:
 *   vp run controller:codegen
 */
...
// Path configuration
const CONTROLLER_SCHEMA_PATH = path.join(__dirname, '../../packages/shared-schema/src/schema/controller.ts');
const OUTPUT_DIR = process.env.GRAPHQL_TYPES_OUTPUT_DIR
  ? path.resolve(process.env.GRAPHQL_TYPES_OUTPUT_DIR)
  : path.join(__dirname, '../libs/graphql-types/src');
...
/**
 * Reject incomplete schema input before emitting a header that can only fail
 * later during C++ compilation. The public schema.ts file is now a re-export,
 * so this also guards against accidentally pointing the text parser at it.
 */
function validateControllerTypes(types) {
  const missingTypes = CONTROLLER_TYPES.filter((typeName) => !types.has(typeName));
  const emptyObjectTypes = CONTROLLER_TYPES.filter((typeName) => {
    const type = types.get(typeName);
    return type && type.kind !== 'union' && type.fields.length === 0;
  });

  if (missingTypes.length > 0 || emptyObjectTypes.length > 0) {
    const problems = [];
    if (missingTypes.length > 0) problems.push(`missing: ${missingTypes.join(', ')}`);
    if (emptyObjectTypes.length > 0) problems.push(`empty: ${emptyObjectTypes.join(', ')}`);
    throw new Error(`Controller GraphQL schema is incomplete (${problems.join('; ')})`);
  }
 ...
 *
 * Source: packages/shared-schema/src/schema/controller.ts
 *
...
async function main() {
  ...
  // Read schema
  console.log(`Reading schema from: ${CONTROLLER_SCHEMA_PATH}`);
  if (!fs.existsSync(CONTROLLER_SCHEMA_PATH)) {
    throw new Error(`Schema file not found at ${CONTROLLER_SCHEMA_PATH}`);
  }
